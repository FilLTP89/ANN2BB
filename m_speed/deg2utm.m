function  [varargout] = deg2utm(varargin)
    ...
    LAT=varargin{1}; n1=length(LAT); % latitude vector
    LON=varargin{2}; n2=length(LON); % longitude vector
    if (n1~=n2)
    ...
    SquareEccentricity=Eccentricity^2; c=(sa^2)/sb;
    Flattening=(sa-sb)/sa; InverseFlattening=1/Flattening;
    k0=0.9996; n=0.00167419; rm=sqrt(sa*sb);
    ...
    E_UTM=zeros(n1,1); N_UTM=zeros(n1,1); utmzone(n1,:)='60 X';
    ...
    for i=1:n1
        ...
        if (lat<-72), Letra='C';
        elseif (lat<-64), Letra='D';
        elseif (lat<-56), Letra='E';
        elseif (lat<-48), Letra='F';
        elseif (lat<-40), Letra='G';
        elseif (lat<-32), Letra='H';
        elseif (lat<-24), Letra='J';
        elseif (lat<-16), Letra='K';
        elseif (lat<-8), Letra='L';
        elseif (lat<0), Letra='M';
        elseif (lat<8), Letra='N';
        elseif (lat<16), Letra='P';
        elseif (lat<24), Letra='Q';
        elseif (lat<32), Letra='R';
        elseif (lat<40), Letra='S';
        elseif (lat<48), Letra='T';
        elseif (lat<56), Letra='U';
        elseif (lat<64), Letra='V';
        elseif (lat<72), Letra='W';
        else Letra='X';
        end
        ...
        E_UTM(i)=Easting;
        N_UTM(i)=Northing;
        utmzone(i,:)=sprintf('%02d %c',geozone,Letra);
    end
    varargout{1}=E_UTM;
    varargout{2}=N_UTM;
    varargout{3}=utmzone;
