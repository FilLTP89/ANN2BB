...
%
% 2) FILENAME.mate
%    File containing: Material properties, Boundary conditions, Fault Rupture
%    Not-Honoring, Non linear Dynamics
%
% 3) LS.input
...
if (SCENARIO ~= 0)
    ...
    FAULT.FO_x = src_par(6);                 % X coor of Fault Origin F0 (zero strike and zero dip) [m]
    FAULT.FO_y = src_par(7);                 % Y coor of Fault Origin F0 (zero strike and zero dip) [m]
    FAULT.FO_z = src_par(8);                 % Z coor of fault Origin F0 (zero strike and zero dip)  [m]
    ...
    FAULT.RAKE0 = src_par(14);               % rake angle (for scenario 0) [deg]
...
TIMESTEP = data.DELTAT;  %1.000e-003;          %Time step used for the time scheme
TMONITOR = data.TMONITOR; %10;                  %The solution is saved every TMONITOR step
...
for k=1:nmate
    tline = fgetl(fid); 
    mate_no = str2num(tline(1:10));
    SD  = str2num(tline(11:20));
    rho = str2num(tline(21:30));
    Vp = str2num(tline(31:40));
    Vs = str2num(tline(41:50));
    Qs = str2num(tline(51:60));
    fmax = str2num(tline(61:70));
...
%     fpeak = str2num(tline(71:80));  
%     nle_yes_not = str2num(tline(81:90));
%     nh_yes_not = str2num(tline(91:100));
    
    nu = Vs^2*rho;
    lambda = Vp^2*rho-2*nu;
    gamma = pi*fpeak/Qs;
    ...
    mate(k,5) = nu;
    mate(k,6) = gamma;
    mate(k,7) = nle_yes_not;
    mate(k,8) = nh_yes_not;
    mate(k,9) = fpeak;
    
    fprintf(fidw,['MATE    %d    %d    %4.2f    %e    %e    %e\n'],mate(k,1),...
        mate(k,2),mate(k,3),mate(k,4),...
        mate(k,5),mate(k,6));
...
if(DGMODEL == 1)
    for i = mate_no+2 : nmate_all
        fprintf(fidw,['DGIC    %d %d'],[i mod(i - mate_no+1,2)]);
        fprintf(fidw,'\n');
    end
end
...
scen_generator(path,path_f,path_out,dirSeparator,utmzone,loc_str,...
    ...
    TAG_FUNC_STF,SCEN,FAULT,data.FOLDER)
%
% fclose(fidw);
%
...
fprintf(fidw3,'TIMESTEP   %e\n',TIMESTEP);
fprintf(fidw3,'TMONITOR   %6.2f\n',TMONITOR);
