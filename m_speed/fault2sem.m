function cirella_fault_geometry(utmzone,loc_str,file_mate,fault_str,SCENARIO,...
%         nfunc,SCEN,FAULT)
%     
%     path = './';
%     path_f = './';
...
%     prop = load([char(path),'sism',filesep,prop_par]);
%     dim = size(prop);
%     nlay = dim(1);
%     
...
%     % SLIP DISTRIBUTION
%     nomeslip = [cd,filesep,'SCENARIOS',filesep,'slip_models.dat'];
...
%     if icrsp == 1
%         nomecrsp = [cd,filesep,'SCENARIOS',filesep,'crsp.dat'];
...
%     
%     
%     nomefiler=[char(path),'sism',filesep,char(file_name),'.txt'];
%     mkdir([char(path_out),filesep,'sism']);
%     nomefilew4=[char(path_out),filesep,'sism',filesep,char(eqk_str),'.SISM'];
%     nomefilew5=[char(path_out),filesep,'sism',filesep,char(eqk_str),'.param'];
%     nomefilew6=[char(path),'scenario_',fault_str,'.param'];
%     nomefile_mate=[char(path),dirname,filesep,'input4speed',filesep,char(file_mate),'.mate'];
%     
...
%     
%     % ====================================================================== %
%     %    3D rototation & translation to map the fault onto the X-Y plane     %
%     % ====================================================================== %
%     % disp([' '])
...
%         %     vel_percentage = 0;
%         %     vel_rnd = velocity.*one.*(1+random('Normal',0,1,1,ctria).*vel_percentage);
%         %     vel_rnd_mod = vel_rnd;
%         %     for i = 1:ctria
%         %         if vel_rnd(i) > vel_threshold
