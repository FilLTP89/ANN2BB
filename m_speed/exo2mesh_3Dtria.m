function exo2mesh_3Dtria(varargin)
tstart=cputime;
...
nomefilew=[char(path),char(file_name),'.post.msh'];
nomefilew2=[char(path),char(file_name),'.input'];
nomefilew3=[char(path),char(file_name),'.out'];
...
fid = fopen(nomefilew2,'w'); 
    ...
    if grid>0
            fprintf(fid,'%i  %+13.7e  %+13.7e  %+13.7e\n',[[1:grid]; grid_x(1:grid)'; grid_y(1:grid)'; grid_z(1:grid)']);
    end
fclose(fid);
