...
% Copyright 2016_
% _super_wgs2utm_ : Function to convert lat/lon vectors into UTM coordinates (WGS84).
%% INPUT
% * _LON (vector of longitude coordinates in decimal degrees)_
% * _LAT (vector of latitude  coordinates in decimanl degrees)_
%% OUTPUT
...
function  [varargout] = super_wgs2utm(varargin)
    ...
    for i=1:n_lat
        ...
        tau1=tan(conflat2);
        csi1=atan(tau1/cos(dlon));
        eta1=asinh(sin(dlon)/sqrt(tau1^2+cos(dlon)^2));
        csi = csi1; 
        eta = eta1;
        for ij=1:7
            csi=csi+ALPHA(ij)*sin(2*ij*csi1)*cosh(2*ij*eta1);
            eta=eta+ALPHA(ij)*cos(2*ij*csi1)*sinh(2*ij*eta1);
        end
        Easting  = E0 + sign(lon*180/pi-Smeridian)*k0*AA*eta;
        Northing = k0*AA*csi;
        if flag_hemis
