...
if any(ismember('x',mon.cp))||any(ismember('y',mon.cp))
    ann.hrz.fn = fullfile(wd,'training','net_075s.mat');
    ann.hrz.TnC  = 0.75;
    ann.hrz = nn_parser(ann);
end
% _parse trained ANN network - vertical directions_
if any(ismember('z',mon.cp))
    ann.vrt.fn = fullfile(wd,'training','net_075s.mat');
    ann.vrt.TnC  = 0.75;
    ann.vrt = nn_parser(0.75,fullfile(wd,'training','net_075s.mat'));
end
%%
% _apply trained ANN on hybrid accelerograms_
