...
!> @brief Computes number of DRM elements nodes without DRM boundary nodes 
!! @author 
!> @date August, 2015
!> @version 1.0
...
!> @param[out] nnode_MDRM number of DRM element nodes without boundary DRM nodes


      subroutine DIME_MDRM_NODE(ii_node_DRM,node_DRM,nnode_BD,& 
	                           node_BD,nnode_MDRM)
...
!    Author: Laura Scandella
	...
	integer*4 :: ii_node_DRM,nnode_BD
	integer*4, dimension(ii_node_DRM) :: node_DRM
    integer*4, dimension(nnode_BD) :: node_BD

    integer*4 :: nnode_MDRM

	integer*4 :: i,j,pnod,check
    
    nnode_MDRM = 0

!   Number of DRM element nodes without boundary DRM nodes (nnode_MDRM)
    do j = 1,ii_node_DRM
	   pnod = node_DRM(j) 
	   check = 0
	   do i = 1,nnode_BD
	   ...
	   enddo
	   if (check.ne.1) then
	       nnode_MDRM = nnode_MDRM+1
	   endif
	enddo

	return

    end subroutine DIME_MDRM_NODE