...
!> @param[in] glob_drm_y
      ...
      subroutine TIME_LOOP_NEW(nnt,xs,ys,cs_nnz,cs,&                                                                          ! 5
...
!********************************************************************************************
      ...
      allocate(u1(2*nnt), u2(2*nnt), fk(2*nnt), fe(2*nnt), fd(2*nnt), sism(2*nnt))                          ! I changed back the allocation to the one in original SPEED 24.11.2015 
      allocate(vel(2*nnt),acc(2*nnt))                                                              
	  
	  allocate(update_index_el_az(2*nnt))
	  
	  do iaz = 1,2*nnt                                                                                      ! I changed the allocation with respect to GeoELSE 24.11.2015 
...
!-----------------------------------------------------------------------------------------------------
         ...
         !do fn = 1,nf
         !   func_value(fn) = get_func_value(nf,func_type,func_indx,func_data,fn,dt,0.0d0)
         !enddo
