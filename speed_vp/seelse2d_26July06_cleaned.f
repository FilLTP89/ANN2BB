...
!     © POLIMI, 1999-2005, All Rights Reserved
!     
!     Disclaimer:
...
!     MECHANICAL VARIABLES
	  ...
	  real*8, dimension (:,:), allocatable :: val_vpsd_l_el	!Marco 18May2005 - Speed-up changes 6% improvement
	  ...
	  real*8, dimension (:), allocatable :: x1_monitorl,y1_monitorl ! Marco - MONITOR LINE - 14Feb2005 
	  real*8, dimension (:), allocatable :: x2_monitorl,y2_monitorl ! Marco - MONITOR LINE - 14Feb2005 
	  ...
	  integer*4, dimension (:), allocatable :: el_monitor						! Marco - Monitored nodes in generic position - 27 March 2005 Easter
      real*8, dimension (:), allocatable :: xr_monitor,yr_monitor,zr_monitor	! Marco - Monitored nodes in generic position - 27 March 2005 Easter	
	  integer*4, dimension (:),   allocatable :: n_monitorl						! Marco - MONITOR LINE - 14Feb2005 
      
      integer*4 :: ns,nn,nn2
      integer*4 :: im,ie,i,j,in,ic,id
      integer*4 :: nts,nsnaps,nmonitors
	  integer*4 :: nmonitorsl													! Marco - MONITOR LINE - 14Feb2005
	  !integer*4, dimension(:), allocatable :: num_monitorsl					! Marco - MONITOR LINE - 14Feb2005
	  integer*4 :: num_monitorsl_sum											! Marco - MONITOR LINE - 14Feb2005
	  real*8, dimension(:), allocatable :: x_monitorl,y_monitorl				! Marco - MONITOR LINE - 14Feb2005 
...
!   DAMPING MATRIX VARIABLES
      ...
      real*8 :: ndt_mon
	  real*8 :: ndt_ch_mon,time_ch_smp ! 25Sep05

!   NON LOCAL VARIABLES CLAY

	  integer*4 :: check_nloc_vpcl											!M&C NON LOCAL 02Mar05 
	  integer*4 :: sum_end_nloc_vpcl										!M&C NON LOCAL 02Mar05 
	  integer*4, dimension(:,:), allocatable :: begin_nloc_vpcl				!M&C NON LOCAL 02Mar05 
	  integer*4, dimension(:,:), allocatable :: end_nloc_vpcl				!M&C NON LOCAL 02Mar05
	  integer*4, dimension(:,:), allocatable :: connectivity_nloc_vpcl		!M&C NON LOCAL 02Mar05 
	  real*8, dimension(:,:), allocatable :: weigth_nloc_vpcl				!M&C NON LOCAL 02Mar05
	  real*8, dimension(:,:), allocatable :: integral_nloc_vpcl				!dead line
	  integer*4, dimension(:,:), allocatable :: iq_nloc_vpcl,ip_nloc_vpcl	!dead line  


!   NON LOCAL VARIABLES SAND

	  integer*4 :: check_nloc_vpsa											!M&C NON LOCAL 02Mar05 
	  integer*4 :: sum_end_nloc_vpsa										!M&C NON LOCAL 02Mar05 
	  integer*4, dimension(:,:), allocatable :: begin_nloc_vpsa				!M&C NON LOCAL 02Mar05 
	  integer*4, dimension(:,:), allocatable :: end_nloc_vpsa				!M&C NON LOCAL 02Mar05
	  integer*4, dimension(:,:), allocatable :: connectivity_nloc_vpsa		!M&C NON LOCAL 02Mar05 
	  real*8, dimension(:,:), allocatable :: weigth_nloc_vpsa				!M&C NON LOCAL 02Mar05
	  real*8, dimension(:,:), allocatable :: integral_nloc_vpsa				!dead line
	  integer*4, dimension(:,:), allocatable :: iq_nloc_vpsa,ip_nloc_vpsa	!dead line  
	  real*8, dimension(:), allocatable :: radius_nloc


!   MESH SCALE 
!
!   THIS OPTION INFLUENCE ONLY THE FOLLOWING VARIABLES:
!   DIRICHLET conditions (DIRX, DIRY, DIPX, DIPY)
!   MONITOL
!   MONITOR
!   TIME_STEP
!   TMONITOR
!   xx_macro, yy_macro (spectral element mesh)
!
!   This feature allows the user of scaling the mesh with a constant factor
!   named "factor_scale_mesh". If the parameter is not given then the 
!   "factor_scale_mesh"  does not influence the mesh !!!
!

	  real*8 :: factor_scale_mesh											! Marco - SCALE MESH 11 May 2005
...
!**********************************************************
      ...
      call read_dime_header(head_file,nmonitors,nmonitorsl,nsnaps,nsnapsps) ! Marco ! Marco - MONITOR LINE - 14Feb2005
      
      if (nmonitors.gt.0) then
         allocate(x_monitor(nmonitors),y_monitor(nmonitors))
		 allocate(el_monitor(nmonitors))	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
         allocate(xr_monitor(nmonitors))	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
         allocate(yr_monitor(nmonitors))	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
         !allocate(n_monitor(nmonitors))	! Marco - MONITOR LINE - 14Feb2005
      endif

	  if (nmonitorsl.gt.0) then										! Marco - MONITOR LINE - 14Feb2005
		 allocate(x1_monitorl(nmonitorsl),y1_monitorl(nmonitorsl))	! Marco - MONITOR LINE - 14Feb2005
		 allocate(x2_monitorl(nmonitorsl),y2_monitorl(nmonitorsl))	! Marco - MONITOR LINE - 14Feb2005
         allocate(n_monitorl(nmonitorsl))							! Marco - MONITOR LINE - 14Feb2005
      endif															! Marco - MONITOR LINE - 14Feb2005
      
      if (nsnaps.gt.0) then
        allocate(tsnap(nsnaps),itersnap(nsnaps))
      endif

	  if (nsnapsps.gt.0) then                              ! Clara&Marco Pre-Stress 9May05
        allocate(tsnapps(nsnapsps),itersnapps(nsnapsps))   ! Clara&Marco Pre-Stress 9May05
      endif                                                ! Clara&Marco Pre-Stress 9May05
      

	  factor_scale_mesh = 0.0d0							   ! Marco - SCALE MESH 11 May 2005
      ...
      call read_header(head_file,grid_file,mat_file,out_file,&
					   ...
					   nmonitorsl,x1_monitorl,y1_monitorl,&		! Marco - MONITOR LINE - 14Feb2005
					   x2_monitorl,y2_monitorl,&				! Marco - MONITOR LINE - 14Feb2005
					   factor_scale_mesh,&						! Marco - SCALE MESH 11 May 2005
					   ndt_ch_mon,time_ch_smp)					!Marco 25Sep05

	  if (factor_scale_mesh.ne.0.0d0) then							! Marco - SCALE MESH 11 May 2005
		call scale_header(deltat,ndt_mon,&							! Marco - SCALE MESH 11 May 2005
		                  nmonitors,x_monitor,y_monitor,&			! Marco - SCALE MESH 11 May 2005
						  nmonitorsl,x1_monitorl,y1_monitorl,&		! Marco - SCALE MESH 11 May 2005
						  x2_monitorl,y2_monitorl,&					! Marco - SCALE MESH 11 May 2005
						  factor_scale_mesh,&						! Marco - SCALE MESH 11 May 2005
						  ndt_ch_mon)								! Marco 25Sep05
	  endif															! Marco - SCALE MESH 11 May 2005
	  ...
	  if (nload_vpcl_el.gt.0) then !Clara
         allocate (val_vpcl_el(nload_vpcl_el,9)) !Clara !M&C NON LOCAL 02Mar05 
		 ! val_vpcl_el(nload_vpcl_el,7) -> Radius of Gaussian evolution probability distrib. func.
	  ...
	  if (nload_vpsd_el.gt.0) then !MCNAP
		 ...
		 allocate (val_vpsd_l_el(nload_vpsd_el,14)) !Marco 18May2005 - Speed-up changes 6% improvement
      endif !MCNAP
	  ...
	  if (factor_scale_mesh.ne.0.0d0) then						! Marco - SCALE MESH 11 May 2005	
		call scale_material_el(nload_dirX_el,val_dirX_el, &		! Marco - SCALE MESH 11 May 2005
					nload_dirY_el,val_dirY_el, &				! Marco - SCALE MESH 11 May 2005
					nload_dipX_el,val_dipX_el, &				! Marco - SCALE MESH 11 May 2005
					nload_dipY_el,val_dipY_el, &				! Marco - SCALE MESH 11 May 2005
					nload_vpcl_el,val_vpcl_el, &				! Marco - SCALE MESH 11 May 2005
					nload_vpsa_el,val_vpsa_el, &				! Marco - SCALE MESH 11 May 2005
					factor_scale_mesh)							! Marco - SCALE MESH 11 May 2005
      endif														! Marco - SCALE MESH 11 May 2005
	  ...
	  if (myid.eq.0) then
         ...
         do im = 1,nload_vpcl_el
			...
			write(*,'(A,E12.4)') 'Radius non loc. [m]..: ',val_vpcl_el(im,7) ! M&C NON LOCAL 02Mar05
            write(*,'(A,E12.4)') 'loading_slopeCamClay.: ',val_vpcl_el(im,8) !Cam_Clay
			write(*,'(A,E12.4)') 'unlaoding_slopeCamCl.: ',val_vpcl_el(im,9) !Cam_Clay 
            write(*,*)

			if (factor_scale_mesh.ne.0.0d0) then							! Marco - SCALE MESH 11 May 2005	
				val_vpcl_el(im,7) = val_vpcl_el(im,7) * factor_scale_mesh	! Marco - SCALE MESH 11 May 2005	
			endif															! Marco - SCALE MESH 11 May 2005	
	  ...
	  if (myid.eq.0) then
         ...
         do im = 1,nload_vpsl_el
            ...
            write(*,'(A,E12.4)') 'u7=pc*sqrt(3)...[Pa].: ',val_vpsa_el(im,2)
            write(*,'(A,E12.4)') 'Radius non loc. [m]..: ',val_vpsa_el(im,3)
            write(*,'(A,E12.4)') 'yield threshold......: ',val_vpsa_el(im,4)  !Clara-new definition nucleus viscous
			write(*,'(A,E12.4)') 'XXXXXXXXXXXXXXXXXXXX.: ',val_vpsa_el(im,5)
            write(*,*)

			if (factor_scale_mesh.ne.0.0d0) then							! Marco - SCALE MESH 11 May 2005	
				val_vpsa_el(im,3) = val_vpsa_el(im,3) * factor_scale_mesh	! Marco - SCALE MESH 11 May 2005	
			endif															! Marco - SCALE MESH 11 May 2005	
	  ...
	  if (myid.eq.0) then
         ...
         do im = 1,nload_vpsl_el
			...
			write(*,'(A,E12.4)') 'dvisco_l.............: ',val_vpsl_el(im,14)   !Clara-new definition nucleus viscous
			write(*,'(A,E12.4)') 'emax.................: ',val_vpsl_el(im,15)
	  ...
	  if (myid.eq.0) then
         ...
         do im = 1,nload_vpsd_el
			...
			write(*,'(A,E12.4)') 'dvisco_d.............: ',val_vpsd_el(im,14)  !Clara-new definition nucleus viscous
			write(*,'(A,E12.4)') 'emin.................: ',val_vpsd_el(im,15)
			write(*,'(A,E12.4)') 'XXXXXXXXXXXXXXXXXXXX.: ',val_vpsd_el(im,16)
            write(*,*)

			val_vpsd_l_el(im,1) = val_vpsd_el(im,1) - val_vpsl_el(im,1)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,2) = val_vpsd_el(im,2) - val_vpsl_el(im,2)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,3) = val_vpsd_el(im,3) - val_vpsl_el(im,3)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,4) = val_vpsd_el(im,4) - val_vpsl_el(im,4)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,5) = val_vpsd_el(im,5) - val_vpsl_el(im,5)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,6) = val_vpsd_el(im,6) - val_vpsl_el(im,6)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,7) = val_vpsd_el(im,7) - val_vpsl_el(im,7)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,8) = val_vpsd_el(im,8) - val_vpsl_el(im,8)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,9) = val_vpsd_el(im,9) - val_vpsl_el(im,9)			!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,10) = val_vpsd_el(im,10) - val_vpsl_el(im,10)		!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,11) = val_vpsd_el(im,11) - val_vpsl_el(im,11)		!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,12) = val_vpsd_el(im,12) - val_vpsl_el(im,12)		!Marco 18May2005 - Speed-up changes 6% improvement
			val_vpsd_l_el(im,13) = val_vpsd_el(im,13) - val_vpsl_el(im,13)		!Marco 18May2005 - Speed-up changes 6% improvement
            val_vpsd_l_el(im,14) = val_vpsd_el(im,14) - val_vpsl_el(im,14)      !Clara-new definition nucleus viscous
...
!     Set initial pre-stress condition
	  ...
	  if (factor_scale_mesh.ne.0.0d0) then									! Marco - SCALE MESH 11 May 2005	
		call scale_grid_el(nnod_macro,xx_macro,yy_macro,factor_scale_mesh)	! Marco - SCALE MESH 11 May 2005	
	  endif																	! Marco - SCALE MESH 11 May 2005	
...
! Dual connectivity for spectral nodes
	  ...
	  if (nmonitorsl.gt.0) then

		call dime_monitorsl(nnod,xx_spx,yy_spx,&					! Marco - MONITOR LINE - 14Feb2005
							num_monitorsl_sum,&						! Marco - MONITOR LINE - 14Feb2005
							nmonitorsl,x1_monitorl,y1_monitorl,&	! Marco - MONITOR LINE - 14Feb2005
							x2_monitorl,y2_monitorl)				! Marco - MONITOR LINE - 14Feb2005

        !allocate(num_monitorsl(nmonitorsl))									! Marco - MONITOR LINE - 14Feb2005
		allocate(x_monitorl(num_monitorsl_sum),y_monitorl(num_monitorsl_sum))	! Marco - MONITOR LINE - 14Feb2005

		call read_monitorsl(nnod,xx_spx,yy_spx,&					! Marco - MONITOR LINE - 14Feb2005
							!num_monitorsl,&						! Marco - MONITOR LINE - 14Feb2005
							num_monitorsl_sum,&						! Marco - MONITOR LINE - 14Feb2005
							x_monitorl,y_monitorl,&					! Marco - MONITOR LINE - 14Feb2005
							nmonitorsl,x1_monitorl,y1_monitorl,&	! Marco - MONITOR LINE - 14Feb2005
							x2_monitorl,y2_monitorl)				! Marco - MONITOR LINE - 14Feb2005

		allocate(n_monitor(nmonitors + num_monitorsl_sum))			! Marco - MONITOR LINE - 14Feb2005
...
!     ELASTIC MATRICES 
	 ...
	 !write(50,*),im,'| prop_mat =',prop_mat(im,4)
	  ...
	  do im = 1,nload_vpcl_el                                        ! M&C NON LOCAL 02Mar05
		if (val_vpcl_el(im,7).gt.0) then                             ! M&C NON LOCAL 02Mar05
			check_nloc_vpcl = 1                                           ! M&C NON LOCAL 02Mar05
			sum_end_nloc_vpcl = 0                                         ! M&C NON LOCAL 02Mar05
		endif                                                        ! M&C NON LOCAL 02Mar05
	  enddo                                                          ! M&C NON LOCAL 02Mar05
																	 ! M&C NON LOCAL 02Mar05
	  if (check_nloc_vpcl.gt.0) then										 ! M&C NON LOCAL 02Mar05
		allocate(begin_nloc_vpcl(nload_vpcl_el,nnod))					 ! M&C NON LOCAL 02Mar05
		allocate(end_nloc_vpcl(nload_vpcl_el,nnod))						 ! M&C NON LOCAL 02Mar05
		allocate(radius_nloc(nload_vpcl_el))
	  endif															 ! M&C NON LOCAL 02Mar05
      ...
      do im = 1,nload_vpcl_el                                        ! M&C NON LOCAL 02Mar05
		...
		radius_nloc(im) = val_vpcl_el(im,7)

		if (val_vpcl_el(im,7).gt.0) then                             ! M&C NON LOCAL 02Mar05
																	 ! M&C NON LOCAL 02Mar05
			if (myid.eq.0) write(*,'(A)')							 ! M&C NON LOCAL 02Mar05
			if (myid.eq.0) write(*,'(A)')'Building the non local connectivity...'	! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
			call dime_nonloc(nnod,xx_spx,yy_spx,node_index,con_nnz,con_spx,&	! M&C NON LOCAL 02Mar05
                    nmat,tag_mat,type_mat,sdeg_mat,tref_mat,prop_mat,&			! M&C NON LOCAL 02Mar05
                    nelem,&														! M&C NON LOCAL 02Mar05
                    con_nnz_bc,con_spx_bc,&										! M&C NON LOCAL 02Mar05
                    xx_macro,yy_macro,con,nelem,con_bc,nedge,&					! M&C NON LOCAL 02Mar05
					nnode_dom,&													! M&C NON LOCAL 02Mar05
					nload_vpcl_el,val_vpcl_el(im,7),tag_vpcl_el,&						! M&C NON LOCAL 02Mar05
					begin_nloc_vpcl,end_nloc_vpcl)										! M&C NON LOCAL 02Mar05		
																				! M&C NON LOCAL 02Mar05
			!call create_connect_weigth_nonloc									! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
			sum_end_nloc_vpcl = end_nloc_vpcl(im,nnod) + sum_end_nloc_vpcl                     ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		endif                                                                   ! M&C NON LOCAL 02Mar05
      enddo
	  endif                                                                    ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
	  if (check_nloc_vpcl.eq.1) then                                                 ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		allocate(connectivity_nloc_vpcl(nload_vpcl_el,sum_end_nloc_vpcl))                 ! M&C NON LOCAL 02Mar05
		allocate(weigth_nloc_vpcl(nload_vpcl_el,sum_end_nloc_vpcl))                       ! M&C NON LOCAL 02Mar05
		allocate(integral_nloc_vpcl(nload_vpcl_el,nnod))                             ! dead line
		allocate(iq_nloc_vpcl(nload_vpcl_el,sum_end_nloc_vpcl))                           ! dead line
		allocate(ip_nloc_vpcl(nload_vpcl_el,sum_end_nloc_vpcl))                           ! dead line
																				! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		call create_connect_weigth_nonloc(nnod,xx_spx,yy_spx,node_index,con_nnz,con_spx,&    ! M&C NON LOCAL 02Mar05
						 nmat,tag_mat,type_mat,sdeg_mat,tref_mat,prop_mat,&                  ! M&C NON LOCAL 02Mar05
						 nelem,alfa1,beta1,gamma1,alfa2,beta2,gamma2,&                       ! M&C NON LOCAL 02Mar05
						 con_nnz_bc,con_spx_bc,&                                             ! M&C NON LOCAL 02Mar05
						 xx_macro,yy_macro,con,nelem,con_bc,nedge,&                          ! M&C NON LOCAL 02Mar05
						 nnode_dom,&                                                         ! M&C NON LOCAL 02Mar05 
						 nload_vpcl_el,radius_nloc,tag_vpcl_el,&                             ! M&C NON LOCAL 02Mar05
						 begin_nloc_vpcl,end_nloc_vpcl,&                                               ! M&C NON LOCAL 02Mar05
						 sum_end_nloc_vpcl,&                                                      ! M&C NON LOCAL 02Mar05
						 connectivity_nloc_vpcl,weigth_nloc_vpcl,&                                     ! M&C NON LOCAL 02Mar05
						 iq_nloc_vpcl,ip_nloc_vpcl,&													 ! M&C NON LOCAL 02Mar05
						 integral_nloc_vpcl)
		
		deallocate(radius_nloc)													     ! M&C NON LOCAL 02Mar05
																							 ! M&C NON LOCAL 02Mar05
	  endif                                                                                  ! M&C NON LOCAL 02Mar05
      ...
      do im = 1,nload_vpsa_el                                        ! M&C NON LOCAL 02Mar05
		if (val_vpsa_el(im,3).gt.0) then                             ! M&C NON LOCAL 02Mar05
			check_nloc_vpsa = 1                                           ! M&C NON LOCAL 02Mar05
			sum_end_nloc_vpsa = 0                                         ! M&C NON LOCAL 02Mar05
		endif                                                        ! M&C NON LOCAL 02Mar05
	  enddo                                                          ! M&C NON LOCAL 02Mar05
																	 ! M&C NON LOCAL 02Mar05
	  if (check_nloc_vpsa.gt.0) then										 ! M&C NON LOCAL 02Mar05
		allocate(begin_nloc_vpsa(nload_vpsa_el,nnod))					 ! M&C NON LOCAL 02Mar05
		allocate(end_nloc_vpsa(nload_vpsa_el,nnod))						 ! M&C NON LOCAL 02Mar05
		allocate(radius_nloc(nload_vpsa_el))
	  endif															 ! M&C NON LOCAL 02Mar05
			
	  if (check_nloc_vpsa.gt.0) then														 ! M&C NON LOCAL 02Mar05
      do im = 1,nload_vpsa_el                                        ! M&C NON LOCAL 02Mar05
		
		radius_nloc(im) = val_vpsa_el(im,3)
																	 ! M&C NON LOCAL 02Mar05
		if (val_vpsa_el(im,3).gt.0) then                             ! M&C NON LOCAL 02Mar05
																	 ! M&C NON LOCAL 02Mar05
			if (myid.eq.0) write(*,'(A)')							 ! M&C NON LOCAL 02Mar05
			...
			call dime_nonloc(nnod,xx_spx,yy_spx,node_index,con_nnz,con_spx,&	! M&C NON LOCAL 02Mar05
                    ...
                    xx_macro,yy_macro,con,nelem,con_bc,nedge,&					! M&C NON LOCAL 02Mar05
					nnode_dom,&													! M&C NON LOCAL 02Mar05
					nload_vpsa_el,val_vpsa_el(im,3),tag_vpsa_el,&						! M&C NON LOCAL 02Mar05
					begin_nloc_vpsa,end_nloc_vpsa)										! M&C NON LOCAL 02Mar05		
																				! M&C NON LOCAL 02Mar05
			!call create_connect_weigth_nonloc									! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
			sum_end_nloc_vpsa = end_nloc_vpsa(im,nnod) + sum_end_nloc_vpsa                     ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		endif                                                                   ! M&C NON LOCAL 02Mar05
      enddo 
	  endif                                                                    ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
	  if (check_nloc_vpsa.eq.1) then                                                 ! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		allocate(connectivity_nloc_vpsa(nload_vpsa_el,sum_end_nloc_vpsa))                 ! M&C NON LOCAL 02Mar05
		allocate(weigth_nloc_vpsa(nload_vpsa_el,sum_end_nloc_vpsa))                       ! M&C NON LOCAL 02Mar05
		allocate(integral_nloc_vpsa(nload_vpsa_el,nnod))                             ! dead line
		allocate(iq_nloc_vpsa(nload_vpsa_el,sum_end_nloc_vpsa))                           ! dead line
		allocate(ip_nloc_vpsa(nload_vpsa_el,sum_end_nloc_vpsa))                           ! dead line
																				! M&C NON LOCAL 02Mar05
																				! M&C NON LOCAL 02Mar05
		call create_connect_weigth_nonloc(nnod,xx_spx,yy_spx,node_index,con_nnz,con_spx,&    ! M&C NON LOCAL 02Mar05
						 nmat,tag_mat,type_mat,sdeg_mat,tref_mat,prop_mat,&                  ! M&C NON LOCAL 02Mar05
						 ...
						 xx_macro,yy_macro,con,nelem,con_bc,nedge,&                          ! M&C NON LOCAL 02Mar05
						 nnode_dom,&                                                         ! M&C NON LOCAL 02Mar05 
						 nload_vpsa_el,radius_nloc,tag_vpsa_el,&                             ! M&C NON LOCAL 02Mar05
						 begin_nloc_vpsa,end_nloc_vpsa,&                                               ! M&C NON LOCAL 02Mar05
						 sum_end_nloc_vpsa,&                                                      ! M&C NON LOCAL 02Mar05
						 connectivity_nloc_vpsa,weigth_nloc_vpsa,&                                     ! M&C NON LOCAL 02Mar05
						 iq_nloc_vpsa,ip_nloc_vpsa,&													 ! M&C NON LOCAL 02Mar05
						 integral_nloc_vpsa)													     ! M&C NON LOCAL 02Mar05
		
		deallocate(radius_nloc)
																							 ! M&C NON LOCAL 02Mar05
	  endif                                                                                  ! M&C NON LOCAL 02Mar05
...
!     SNAPSHOTS PRESTRESS                                       ! Clara&Marco Pre-Stress 9May05
      
      if (nsnapsps.gt.0) then                                   ! Clara&Marco Pre-Stress 9May05
        do i = 1,nsnapsps                                       ! Clara&Marco Pre-Stress 9May05
          itersnapps(i) = int(tsnapps(i)/deltat)                ! Clara&Marco Pre-Stress 9May05
          if (itersnapps(i).gt.nts) itersnapps(i) = nts         ! Clara&Marco Pre-Stress 9May05
        enddo                                                   ! Clara&Marco Pre-Stress 9May05
      endif                                                     ! Clara&Marco Pre-Stress 9May05
...
!     NODI MONITORATI
      ...
      if (nmonitors.ge.1) then
         do i = 1,nmonitors
			...
			!call find_point_pos(nelem,&						! Marco - Monitored nodes in generic position - 27 March 2005 Easter
			!	 alfa1,beta1,gamma1,&						! Marco - Monitored nodes in generic position - 27 March 2005 Easter
			!	 alfa2,beta2,gamma2,&						! Marco - Monitored nodes in generic position - 27 March 2005 Easter
			!	 delta1,delta2,&							! Marco - Monitored nodes in generic position - 27 March 2005 Easter
            !     x_monitor(i),y_monitor(i),&				! Marco - Monitored nodes in generic position - 27 March 2005 Easter
            !     el_monitor(i),xr_monitor(i),yr_monitor(i))	! Marco - Monitored nodes in generic position - 27 March 2005 Easter

         enddo
      endif

	  if (nmonitorsl.ge.1) then										! Marco - MONITOR LINE - 14Feb2005
         do i = 1,num_monitorsl_sum									! Marco - MONITOR LINE - 14Feb2005
            call find_nearest_node(nnod,xx_spx,yy_spx,&				! Marco - MONITOR LINE - 14Feb2005
                                   x_monitorl(i),y_monitorl(i),&	! Marco - MONITOR LINE - 14Feb2005
								   n_monitor(i+nmonitors))			! Marco - MONITOR LINE - 14Feb2005
         enddo														! Marco - MONITOR LINE - 14Feb2005
		!nmonitors = nmonitors + num_monitorsl_sum					! Marco - MONITOR LINE - 14Feb2005
      endif															! Marco - MONITOR LINE - 14Feb2005
	  ...
	  if (myid.eq.0) then
		...
		write(*,'(I6,2E14.5,I6)')(i,x_monitor(i),y_monitor(i),n_monitor(i),&
			  i=1,nmonitors)											! Marco - MONITOR LINE - 14Feb2005

		write(*,'(A)')'Monitored nodes - spectral coordinates'
		write(*,'(I6,2E14.5,I6)')(i,xx_spx(n_monitor(i)),yy_spx(n_monitor(i)),n_monitor(i),&
				 i=1,nmonitors)											! Marco - MONITOR LINE - 14Feb2005

		!write(*,'(A,I8,A,I8,3E10.2)')('Closest node :',n_monitor(i),&	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
        !         ', elem. :',el_monitor(i),&							! Marco - Monitored nodes in generic position - 27 March 2005 Easter
        !         xr_monitor(i),yr_monitor(i),&							! Marco - Monitored nodes in generic position - 27 March 2005 Easter
		!		 i=1,nmonitors)											! Marco - Monitored nodes in generic position - 27 March 2005 Easter
		!write(*,'(A)')													! Marco - Monitored nodes in generic position - 27 March 2005 Easter

	  endif


	  if (nmonitorsl.ge.1) then										! Marco - MONITOR LINE - 14Feb2005
		if (myid.eq.0) then											! Marco - MONITOR LINE - 14Feb2005
			 write(*,'(A)')'Monitored nodes - user coordinates'		! Marco - MONITOR LINE - 14Feb2005
			 write(*,'(I6,2E14.5,I6)')(i,x_monitorl(i),y_monitorl(i),n_monitor(i+nmonitors),& ! Marco - MONITOR LINE - 14Feb2005
			    i=1,num_monitorsl_sum)								! Marco - MONITOR LINE - 14Feb2005
			 
			 write(*,'(A)')'Monitored nodes - spectral coordinates'	! Marco - MONITOR LINE - 14Feb2005
			 write(*,'(I6,2E14.5,I6)')(i,xx_spx(n_monitor(i+nmonitors)),yy_spx(n_monitor(i+nmonitors)),n_monitor(i+nmonitors),&	! Marco - MONITOR LINE - 14Feb2005
				 i=1,num_monitorsl_sum)								! Marco - MONITOR LINE - 14Feb2005
      ...
      call time_loop_el(nnod,xx_spx,yy_spx,node_index,con_nnz,con_spx,&
						...
						nload_vpsl_el,val_vpsl_el,tag_vpsl_el,&						!MCNAP
						nload_vpsd_el,val_vpsd_el,tag_vpsd_el,&						!MCNAP
						val_vpsd_l_el,&												!Marco 18May2005 - Speed-up changes 6% improvement
						nload_maps_el,val_maps_el,fun_maps_el,tag_maps_el,&			! Marco
						nsnapsps,itersnapps,nprocs,& !Marco
						begin_nloc_vpcl,end_nloc_vpcl,&								! M NON LOCAL 02Mar05
				        sum_end_nloc_vpcl,&							   				! M NON LOCAL 02Mar05
						connectivity_nloc_vpcl,weigth_nloc_vpcl,&					! M NON LOCAL 02Mar05 
						iq_nloc_vpcl,ip_nloc_vpcl,&
						integral_nloc_vpcl,&										!dead line
                        begin_nloc_vpsa,end_nloc_vpsa,&								! M NON LOCAL 02Mar05
				        sum_end_nloc_vpsa,&							   				! M NON LOCAL 02Mar05
						connectivity_nloc_vpsa,weigth_nloc_vpsa,&					! M NON LOCAL 02Mar05 
						iq_nloc_vpsa,ip_nloc_vpsa,&
						integral_nloc_vpsa,&	             
						el_monitor,xr_monitor,yr_monitor,&							! Marco - Monitored nodes in generic position - 27 March 2005 Easter
						ndt_ch_mon,time_ch_smp)													!Marco 25Sep05
      ...
      if (nmonitorsl.gt.0) then
		deallocate(x_monitorl,y_monitorl)
	  endif
      if ((nmonitors-num_monitorsl_sum).gt.0) then
		deallocate(x_monitor,y_monitor)
	  endif
	  if ((nmonitorsl.gt.0).or.((nmonitors-num_monitorsl_sum).gt.0)) then
		deallocate(n_monitor)
	  endif
...
!     ********************************************************
      ...
      subroutine time_loop_el(nnt,xs,ys,node_index,cs_nnz,cs,&
							  ...
							  valvpsd_l,&										!Marco 18May2005 - Speed-up changes 6% improvement
							  ...
							  iq_nloc_vpsa,ip_nloc_vpsa,&
							  integral_nloc_vpsa,&	
							  elem_m,xr_m,yr_m,&								! Marco - Monitored nodes in generic position - 27 March 2005 Easter
							  ndt_change_monitor,time_change_sampling)				    ! Marco 25Sep05
...
!     
	  ...
	  integer*4, dimension(nmonit) :: elem_m	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
      real*8, dimension(nmonit) :: xr_m,yr_m	! Marco - Monitored nodes in generic position - 27 March 2005 Easter
	  ...
	  character*70 :: file_force_int ! M "me&you" paper 09.02.2005 -> Output in term of Internal Force history
	  character*70 :: file_force_ext ! Marco 24.04.2005 -> Output in term of External Force history
	  character*70 :: file_strain ! Marco 10.02.2005 "Strain Output" 
	  character*70 :: file_yield ! Marco 4Apr2005 - Output yield
	  character*70 :: file_vnucl ! Marco 4Apr2005 - Output yield
      character*70 :: file_outE
	  character*70 :: file_out_velocity !Marco 16Sep05
	  character*70 :: file_out_acceleration !Marco 16Sep05
      character*70 :: file_outV ! Clara
	  character*70 :: file_outY ! Marco 4Apr2005 - Output yield
	  character*70 :: file_outF ! Marco 4Apr2005 - Output yield
	  character*70 :: file_outT1 ! Clara
	  character*70 :: file_outT2 ! Clara
	  character*70 :: file_outT3 ! Clara
      character*70 :: file_outT4 ! Clara 23/04/05
	  character*70 :: file_outS1 ! Clara
	  character*70 :: file_outS2 ! Clara
	  character*70 :: file_outS3 ! Clara
	  character*70 :: file_outS4 ! Clara
	  ...
	  character*70 :: file_out_yield ! Marco 4Apr2005 - Output yield
	  character*70 :: file_out_vnucl ! Marco 4Apr2005 - Output yield
	  ...
	  character*70 :: file_out_vistn4_0  ! Clara 23/04/05
	  
	  character*70 :: file_out_a1n1   ! Clara SNAPPRES SAND   
	  character*70 :: file_out_a1n2   ! Clara SNAPPRES SAND
	  character*70 :: file_out_a1n3   ! Clara SNAPPRES SAND
	  character*70 :: file_out_a1n4   ! Clara SNAPPRES SAND
	  character*70 :: file_out_a1n5   ! Clara SNAPPRES SAND
	  character*70 :: file_out_a1n6   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn1   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn2   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn3   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn4   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn5   ! Clara SNAPPRES SAND
	  character*70 :: file_out_epn6   ! Clara SNAPPRES SAND
	  character*70 :: file_out_z14n   ! Clara SNAPPRES SAND
	  character*70 :: file_out_u7n    ! Clara SNAPPRES SAND
	  character*70 :: file_out_densit ! Clara SNAPPRES SAND
...
!*************************************************
      ...
      real*8, dimension(:,:), allocatable :: sxx_el,syy_el,sxy_el,szz_el,fx_el,fy_el
...
!*************************************************
	  ...
	  real*8, dimension(:), allocatable :: vel_az,acc_az ! Marco 16Sep05
...
!*************************************************      
	 ...
	 real*8 :: ndt_monitor
	 real*8 :: ndt_change_monitor,time_change_sampling ! Marco 25Sep05
...
!*************************************************
	  ...
	  real*8, dimension(nvpsd,*) :: valvpsd_l !Marco 18May2005 - Speed-up changes 6% improvement
      ...
      real*8, dimension(:,:,:), allocatable :: pcc_el !Cam_Clay
	  real*8, dimension(:,:,:), allocatable :: vivel_el ! Clara 
	  real*8, dimension(:), allocatable :: vivel_az ! Clara
	  real*8, dimension(:,:,:), allocatable :: yield_el ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: yield_az		! Marco 4Apr2005 - Output yield
	  real*8, dimension(:,:,:), allocatable :: vnucl_el ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vnucl_az		! Marco 4Apr2005 - Output yield
	  !integer*4, dimension(:), allocatable :: check_vivel_az ! Clara 
      integer*4, dimension(:), allocatable :: nodal_counter ! Clara 
	  real*8, dimension(:,:,:), allocatable :: vistn1_0,vistn2_0,vistn3_0,vistn4_0 ! Clara 23/04/05 
      ...
      real*8, dimension(:), allocatable :: a1n1_az,a1n2_az,a1n3_az,a1n4_az,a1n5_az,a1n6_az ! Clara SNAPPRES SAND  
      real*8, dimension(:), allocatable :: epn1_az,epn2_az,epn3_az,epn4_az,epn5_az,epn6_az ! Clara SNAPPRES SAND  
	  real*8, dimension(:), allocatable :: z14n_az,u7n_az,densit_az             ! Clara SNAPPRES SAND  

	  real*8, dimension(:), allocatable :: sxx_az,syy_az,sxy_az,szz_az ! Clara
	  real*8, dimension(:), allocatable :: duxdx_az,duydy_az,duxdy_az ! Marco 10.02.2005 "Strain Output" 

	  real*8, dimension(:), allocatable :: vivel_az_1 ! M Mexico _A_
	  real*8, dimension(:), allocatable :: yield_az_1 ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vnucl_az_1 ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vistn1_0_az, vistn2_0_az, vistn3_0_az, vistn4_0_az ! Clara 23/04/05
	  real*8, dimension(:), allocatable :: vistn1_0_az_1, vistn2_0_az_1, vistn3_0_az_1, vistn4_0_az_1 ! Clara 23/04/05
...
!*************************************************
	  ...
	  real*8, dimension(:), allocatable :: yield_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vnucl_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read ! Clara 23/04/05
...
!*************************************************
!
!               NON LOCAL MATERIAL  CLAY
!
!*************************************************



	  integer*4, dimension(nvpcl,nnt) :: end_nloc_vpcl
	  integer*4, dimension(nvpcl,nnt) :: begin_nloc_vpcl
	  integer*4, dimension(nvpcl,nnt) :: check_nloc_1_vpcl
	  integer*4, dimension(nvpcl,nnt) :: check_nloc_2_vpcl
      
	  integer*4 :: sum_end_nloc_vpcl
	  integer*4, dimension(nvpcl,sum_end_nloc_vpcl) :: connectivity_nloc_vpcl
	  integer*4, dimension(nvpcl,sum_end_nloc_vpcl) :: iq_nloc_vpcl
	  integer*4, dimension(nvpcl,sum_end_nloc_vpcl) :: ip_nloc_vpcl
	  real*8, dimension(nvpcl,sum_end_nloc_vpcl) :: weigth_nloc_vpcl
	  real*8, dimension(nvpcl,nnt) :: integral_nloc_vpcl ! dead line
	  integer*4, dimension(nnt) :: check_in_vpcl

	  real*8, dimension(nvpcl,nnt) :: yield1_az_vpcl,yield2_az_vpcl
	  !real*8, dimension(nnt) :: yield_new

!*************************************************

!*************************************************
!
!               NON LOCAL MATERIAL  SAND
!
!*************************************************



	  integer*4, dimension(nvpsa,nnt) :: end_nloc_vpsa
	  integer*4, dimension(nvpsa,nnt) :: begin_nloc_vpsa
	  integer*4, dimension(nvpsa,nnt) :: check_nloc_1_vpsa
	  integer*4, dimension(nvpsa,nnt) :: check_nloc_2_vpsa
      
	  integer*4 :: sum_end_nloc_vpsa
	  integer*4, dimension(nvpsa,sum_end_nloc_vpsa) :: connectivity_nloc_vpsa
	  integer*4, dimension(nvpsa,sum_end_nloc_vpsa) :: iq_nloc_vpsa
	  integer*4, dimension(nvpsa,sum_end_nloc_vpsa) :: ip_nloc_vpsa
	  real*8, dimension(nvpsa,sum_end_nloc_vpsa) :: weigth_nloc_vpsa
	  real*8, dimension(nvpsa,nnt) :: integral_nloc_vpsa ! dead line
	  integer*4, dimension(nnt) :: check_in_vpsa

	  real*8, dimension(nvpsa,nnt) :: yield1_az_vpsa,yield2_az_vpsa
	  !real*8, dimension(nnt) :: yield_new

	  real*8, dimension(:), allocatable :: a1n1_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: a1n2_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: a1n3_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: a1n4_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: a1n5_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: a1n6_read ! Clara SNAPPRES SAND

	  real*8, dimension(:), allocatable :: epn1_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: epn2_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: epn3_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: epn4_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: epn5_read ! Clara SNAPPRES SAND
	  real*8, dimension(:), allocatable :: epn6_read ! Clara SNAPPRES SAND

	  real*8, dimension(:), allocatable :: z14n_read ! Clara SNAPPRES SAND

	  real*8, dimension(:), allocatable :: u7n_read ! Clara SNAPPRES SAND

	  real*8, dimension(:), allocatable :: densit_read ! Clara SNAPPRES SAND
...
!*************************************************
      ...
      file_out_velocity = file_out(1:lname) // 'vel' ! Marco 16Sep05
	  file_out_acceleration = file_out(1:lname) // 'acc' ! Marco 16Sep05
      file_outV = file_out(1:lname) // 'V' ! Clara
	  file_outY = file_out(1:lname) // 'Y' ! Marco 4Apr2005 - Output yield
	  file_outF = file_out(1:lname) // 'F' ! Marco 4Apr2005 - Output yield
	  file_outT1 = file_out(1:lname) // '1' ! Clara
	  file_outT2 = file_out(1:lname) // '2' ! Clara
	  file_outT3 = file_out(1:lname) // '3' ! Clara
      file_outT4 = file_out(1:lname) // '8' ! Clara 23/04/05
	  file_outS1 = file_out(1:lname) // '4' ! Clara
	  file_outS2 = file_out(1:lname) // '5' ! Clara
	  file_outS3 = file_out(1:lname) // '6' ! Clara
	  file_outS4 = file_out(1:lname) // '7' ! Clara
	  ...
	  file_out_yield = file_out(1:lname) // '_yield' ! Marco 4Apr2005 - Output yield
	  file_out_vnucl = file_out(1:lname) // '_vnucl' ! Marco 4Apr2005 - Output yield

	  ! This output is only for the prestress 23.04.2005
	  file_out_vistn1_0 = file_out(1:lname) // '_vistn1_0' ! M Mexico _A_   xx
	  file_out_vistn2_0 = file_out(1:lname) // '_vistn2_0' ! M Mexico _A_   yy
	  file_out_vistn3_0 = file_out(1:lname) // '_vistn3_0' ! M Mexico _A_   xy
	  file_out_vistn4_0 = file_out(1:lname) // '_vistn4_0' ! Clara 23/04/05 zz

	  file_out_a1n1   = file_out(1:lname) // '_a1n1'  ! Clara SNAPPRES SAND
	  file_out_a1n2   = file_out(1:lname) // '_a1n2'  ! Clara SNAPPRES SAND
	  file_out_a1n3   = file_out(1:lname) // '_a1n3'  ! Clara SNAPPRES SAND
	  file_out_a1n4   = file_out(1:lname) // '_a1n4'  ! Clara SNAPPRES SAND
	  file_out_a1n5   = file_out(1:lname) // '_a1n5'  ! Clara SNAPPRES SAND
	  file_out_a1n6   = file_out(1:lname) // '_a1n6'  ! Clara SNAPPRES SAND
	  file_out_epn1   = file_out(1:lname) // '_epn1'  ! Clara SNAPPRES SAND
	  file_out_epn2   = file_out(1:lname) // '_epn2'  ! Clara SNAPPRES SAND
	  file_out_epn3   = file_out(1:lname) // '_epn3'  ! Clara SNAPPRES SAND
	  file_out_epn4   = file_out(1:lname) // '_epn4'  ! Clara SNAPPRES SAND
	  file_out_epn5   = file_out(1:lname) // '_epn5'  ! Clara SNAPPRES SAND
	  file_out_epn6   = file_out(1:lname) // '_epn6'  ! Clara SNAPPRES SAND
	  file_out_z14n   = file_out(1:lname) // '_z14n'  ! Clara SNAPPRES SAND
	  file_out_u7n    = file_out(1:lname) // '_u7n'   ! Clara SNAPPRES SAND
	  file_out_densit = file_out(1:lname) // '_densit'! Clara SNAPPRES SAND
      ...
      if (nmonit.ge.1) then 
		 ...
		 file_force_int = 'force_intXX.d'					! M "me&you" paper
         do i = 1,nmonit! M "me&you" paper					! M "me&you" paper
            unit_monitor = 20000 + i						! M "me&you" paper
            in = node_m(i)									! M "me&you" paper
            if (node_index(in).ne.0) then					! M "me&you" paper
               if (i.lt.10) then							! M "me&you" paper
                  write(file_force_int(10:10),'(a1)')'0'	! M "me&you" paper
                  write(file_force_int(11:11),'(i1)')i		! M "me&you" paper
               else if (i.le.99) then						! M "me&you" paper
                  write(file_force_int(10:11),'(i2)')i		! M "me&you" paper
               endif										! M "me&you" paper 
               
               open(unit_monitor,file=file_force_int)		! M "me&you" paper
            endif											! M "me&you" paper
         enddo												! M "me&you" paper

		 ! M "me&you" paper


		 ! Marco 10.02.2005 "Strain Output" 

		 file_strain = 'strainXX.d'					! Marco 10.02.2005 "Strain Output" 
         do i = 1,nmonit							! Marco 10.02.2005 "Strain Output" 
            unit_monitor = 30000 + i				! Marco 10.02.2005 "Strain Output" 
            in = node_m(i)							! Marco 10.02.2005 "Strain Output" 
            if (node_index(in).ne.0) then			! Marco 10.02.2005 "Strain Output" 
               if (i.lt.10) then					! Marco 10.02.2005 "Strain Output" 
                  write(file_strain(7:7),'(a1)')'0' ! Marco 10.02.2005 "Strain Output" 
                  write(file_strain(8:8),'(i1)')i	! Marco 10.02.2005 "Strain Output" 
               else if (i.le.99) then				! Marco 10.02.2005 "Strain Output" 
                  write(file_strain(7:8),'(i2)')i	! Marco 10.02.2005 "Strain Output" 
               endif								! Marco 10.02.2005 "Strain Output"              
               open(unit_monitor,file=file_strain)	! Marco 10.02.2005 "Strain Output" 
            endif									! Marco 10.02.2005 "Strain Output" 
         enddo										! Marco 10.02.2005 "Strain Output" 

		 ! Marco 10.02.2005 "Strain Output"
		 
		 
		 file_yield = 'yieldXX.d'						! Marco 4Apr2005 - Output yield
         do i = 1,nmonit								! Marco 4Apr2005 - Output yield
            unit_monitor = 40000 + i					! Marco 4Apr2005 - Output yield
            in = node_m(i)								! Marco 4Apr2005 - Output yield
            if (node_index(in).ne.0) then				! Marco 4Apr2005 - Output yield
               if (i.lt.10) then						! Marco 4Apr2005 - Output yield
                  write(file_yield(6:6),'(a1)')'0'		! Marco 4Apr2005 - Output yield
                  write(file_yield(7:7),'(i1)')i		! Marco 4Apr2005 - Output yield
               else if (i.le.99) then					! Marco 4Apr2005 - Output yield
                  write(file_yield(6:7),'(i2)')i		! Marco 4Apr2005 - Output yield
               endif									! Marco 4Apr2005 - Output yield
               
               open(unit_monitor,file=file_yield)		! Marco 4Apr2005 - Output yield
            endif										! Marco 4Apr2005 - Output yield
         enddo											! Marco 4Apr2005 - Output yield

		 file_vnucl = 'vnuclXX.d'						! Marco 4Apr2005 - Output yield
         do i = 1,nmonit								! Marco 4Apr2005 - Output yield
            unit_monitor = 50000 + i					! Marco 4Apr2005 - Output yield
            in = node_m(i)								! Marco 4Apr2005 - Output yield
            if (node_index(in).ne.0) then				! Marco 4Apr2005 - Output yield
               if (i.lt.10) then						! Marco 4Apr2005 - Output yield
                  write(file_vnucl(6:6),'(a1)')'0'		! Marco 4Apr2005 - Output yield
                  write(file_vnucl(7:7),'(i1)')i		! Marco 4Apr2005 - Output yield
               else if (i.le.99) then					! Marco 4Apr2005 - Output yield
                  write(file_vnucl(6:7),'(i2)')i		! Marco 4Apr2005 - Output yield
               endif									! Marco 4Apr2005 - Output yield
               
               open(unit_monitor,file=file_vnucl)		! Marco 4Apr2005 - Output yield
            endif										! Marco 4Apr2005 - Output yield
         enddo											! Marco 4Apr2005 - Output yield


		 file_force_ext = 'force_extXX.d'					! Marco 24.04.2005
         do i = 1,nmonit									! Marco 24.04.2005
            unit_monitor = 60000 + i						! Marco 24.04.2005
            in = node_m(i)									! Marco 24.04.2005
            if (node_index(in).ne.0) then					! Marco 24.04.2005
               if (i.lt.10) then							! Marco 24.04.2005
                  write(file_force_ext(10:10),'(a1)')'0'	! Marco 24.04.2005
                  write(file_force_ext(11:11),'(i1)')i		! Marco 24.04.2005
               else if (i.le.99) then						! Marco 24.04.2005
                  write(file_force_ext(10:11),'(i2)')i		! Marco 24.04.2005
               endif										! Marco 24.04.2005
               
               open(unit_monitor,file=file_force_ext)		! Marco 24.04.2005
            endif											! Marco 24.04.2005
         enddo												! Marco 24.04.2005
...
!*************************************************
	  ...
	  allocate(vel_az(0:(N_update_el_az + N_extern_el_az -1))) ! Marco 16Sep05
	  allocate(acc_az(0:(N_update_el_az + N_extern_el_az -1))) ! Marco 16Sep05
	  ...
	  allocate(yield_az(0:(N_update_el_az -1))) ! Marco 4Apr2005 - Output yield
	  allocate(vnucl_az(0:(N_update_el_az -1))) ! Marco 4Apr2005 - Output yield
	  
	  allocate(sxx_az(0:(N_update_el_az -1))) ! Clara 
	  allocate(syy_az(0:(N_update_el_az -1))) ! Clara 
	  allocate(sxy_az(0:(N_update_el_az -1))) ! Clara
	  allocate(szz_az(0:(N_update_el_az -1))) ! Clara
	  
	  allocate(duxdx_az(0:(N_update_el_az -1))) ! Marco 10.02.2005 "Strain Output" 
	  allocate(duydy_az(0:(N_update_el_az -1))) ! Marco 10.02.2005 "Strain Output"  
	  allocate(duxdy_az(0:(N_update_el_az -1))) ! Marco 10.02.2005 "Strain Output" 
	  ...
	  allocate(vistn4_0_az(0:(N_update_el_az -1))) ! Clara 23/04/05
	  
	  allocate(vivel_az_1(0:(N_update_el_az -1))) ! M Mexico _A_
	  allocate(yield_az_1(0:(N_update_el_az -1))) ! Marco 4Apr2005 - Output yield
	  allocate(vnucl_az_1(0:(N_update_el_az -1))) ! Marco 4Apr2005 - Output yield
	  ...
	  allocate(vistn4_0_az_1(0:(N_update_el_az -1))) ! Clara 23/04/05
	  ...
	  allocate(yield_read(0:(N_update_el_az + N_extern_el_az -1))) ! Marco 4Apr2005 - Output yield
	  allocate(vnucl_read(0:(N_update_el_az + N_extern_el_az -1))) ! Marco 4Apr2005 - Output yield
	  ...
	  allocate(vistn4_0_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara 23/04/05
      ...
      allocate(a1n1_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND     
      allocate(a1n2_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n3_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n4_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n5_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n6_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND   
                                                          
	  allocate(epn1_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn2_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn3_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn4_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn5_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn6_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND
                                                          
	  allocate(z14n_az(0:(N_update_el_az -1))) ! Clara SNAPPRES SAND 
	  allocate(u7n_az(0:(N_update_el_az -1)))  ! Clara SNAPPRES SAND
	  allocate(densit_az(0:(N_update_el_az -1)))  ! Clara SNAPPRES SAND


	  allocate(a1n1_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND     
      allocate(a1n2_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n3_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n4_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n5_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND   
	  allocate(a1n6_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND   
                                                          
	  allocate(epn1_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn2_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn3_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn4_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn5_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
	  allocate(epn6_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND
                                                          
	  allocate(z14n_read(0:(N_update_el_az + N_extern_el_az -1))) ! Clara SNAPPRES SAND 
	  allocate(u7n_read(0:(N_update_el_az + N_extern_el_az -1)))  ! Clara SNAPPRES SAND
	  allocate(densit_read(0:(N_update_el_az + N_extern_el_az -1)))  ! Clara SNAPPRES SAND
      ...
      do iaz = 0,N_update_el_az -1
		 ...
		 yield_read(iaz) = 0.0d0 ! Marco 4Apr2005 - Output yield
		 vnucl_read(iaz) = 0.0d0 ! Marco 4Apr2005 - Output yield
		 
		 vistn1_0_read(iaz) = 0.0d0 ! M Mexico _A_
		 vistn2_0_read(iaz) = 0.0d0 ! M Mexico _A_
		 vistn3_0_read(iaz) = 0.0d0 ! M Mexico _A_
		 vistn4_0_read(iaz) = 0.0d0 ! Clara 23/04/05

		 u2_star(iaz) = 0.0d0 ! M Mexico _A_
		 fe_star(iaz) = 0.0d0 ! M Mexico _A_

         a1n1_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 a1n2_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND 
		 a1n3_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND 
		 a1n4_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND 
		 a1n5_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND 
		 a1n6_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND   
		 
		 epn1_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 epn2_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 epn3_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 epn4_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 epn5_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 epn6_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND    

		 z14n_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 u7n_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
		 densit_read(iaz) = 0.0d0  ! Clara SNAPPRES SAND
...
!*************************************************	  
      ...
      allocate(pcc_el(nn,nn,nelem_ebe)) !Cam_Clay
	  allocate(vivel_el(nn,nn,nelem_ebe)) ! Clara
	  allocate(yield_el(nn,nn,nelem_ebe)) ! Marco 4Apr2005 - Output yield
	  allocate(vnucl_el(nn,nn,nelem_ebe)) ! Marco 4Apr2005 - Output yield
	  allocate(vistn1_0(nn,nn,nelem_ebe)) ! Clara 
	  allocate(vistn2_0(nn,nn,nelem_ebe)) ! Clara 
	  allocate(vistn3_0(nn,nn,nelem_ebe)) ! Clara 
	  allocate(vistn4_0(nn,nn,nelem_ebe)) ! Clara 23/04/05 
...
!*************************************************
	    ...
	    do ivpcl = 1,nvpcl										! M NON LOCAL 02Mar05
			if (valvpcl(ivpcl,7).gt.0) then						! M NON LOCAL 02Mar05
				do in = 1,nnt									! M NON LOCAL 02Mar05
					yield2_az_vpcl(ivpcl,in) = 0.0d0					! M NON LOCAL 02Mar05
					yield1_az_vpcl(ivpcl,in) = 0.0d0					! M NON LOCAL 02Mar05
					!yield_new(in) = 0.0d0		
				enddo											! M NON LOCAL 02Mar05
			endif												! M NON LOCAL 02Mar05
		 enddo													! M NON LOCAL 02Mar05


       do ivpsa = 1,nvpsa										! M NON LOCAL 02Mar05
			if (valvpsa(ivpsa,3).gt.0) then						! M NON LOCAL 02Mar05
				do in = 1,nnt									! M NON LOCAL 02Mar05
					yield2_az_vpsa(ivpsa,in) = 0.0d0					! M NON LOCAL 02Mar05
					yield1_az_vpsa(ivpsa,in) = 0.0d0					! M NON LOCAL 02Mar05
					!yield_new(in) = 0.0d0		
				enddo											! M NON LOCAL 02Mar05
			endif												! M NON LOCAL 02Mar05
		 enddo													! M NON LOCAL 02Mar05
...
!*************************************************
! Homogeneous and elastic starting condition       

      isnap = 1
	  isnapps = 1						! Clara&Marco Pre-Stress 9May05
...
! *********************************************************
! Visco-plastic variables = 0


	do iaz = 0,N_update_el_az -1
		 
		 vivel_az(iaz) = 0.0d0	! Clara 
		 yield_az(iaz) = 0.0d0	! Marco 4Apr2005 - Output yield
		 vnucl_az(iaz) = 0.0d0	! Marco 4Apr2005 - Output yield
		 
		 sxx_az(iaz) = 0.0d0	! Clara 
		 syy_az(iaz) = 0.0d0	! Clara 
		 sxy_az(iaz) = 0.0d0	! Clara
		 szz_az(iaz) = 0.0d0	! Clara
		 
		 duxdx_az(iaz) = 0.0d0		! Marco 10.02.2005 "Strain Output"  
		 duydy_az(iaz) = 0.0d0		! Marco 10.02.2005 "Strain Output" 
		 duxdy_az(iaz) = 0.0d0		! Marco 10.02.2005 "Strain Output" 
		 
		 vistn1_0_az(iaz)  = 0.0d0		! M Mexico _A_
		 vistn2_0_az(iaz)  = 0.0d0		! M Mexico _A_ 
		 vistn3_0_az(iaz)  = 0.0d0		! M Mexico _A_ 
		 vistn4_0_az(iaz)  = 0.0d0		! Clara 23/04/05


         a1n1_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 a1n2_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 a1n3_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 a1n4_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 a1n5_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 a1n6_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn1_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn2_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn3_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn4_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn5_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 epn6_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 z14n_az(iaz)   = 0.0d0 ! Clara SNAPPRES SAND
		 u7n_az(iaz)    = 0.0d0 ! Clara SNAPPRES SAND
		 densit_az(iaz) = 0.0d0 ! Clara SNAPPRES SAND
	  ...
	  do i = 1,nn ! Clara 
		do j = 1,nn ! Clara 
			do ie = 1,nelem_ebe ! Clara 
				...
				vistn4_0(i,j,ie) = 0.0d0 ! Clara 23/04/05
				vivel_el(i,j,ie) = 0.0d0 ! Clara
				yield_el(i,j,ie) = 0.0d0 ! Marco 4Apr2005 - Output yield
				vnucl_el(i,j,ie) = 0.0d0 ! Marco 4Apr2005 - Output yield
				...
				if (nelem_ebe.gt.0) then   !Cam_Clay

					do im = 1,nm           !Cam_Clay
               
					which_vpcl = 0         !Cam_Clay 

					do ivpcl = 1,nvpcl     !Cam_Clay
						if (tag_mat(im).eq.tagvpcl(ivpcl)) then !Cam_Clay 
							which_vpcl = ivpcl !Cam_Clay
                            pcc_el(i,j,ie) = valvpcl(which_vpcl,1) !Cam_Clay
				...
				if (nelem_ebe.gt.0) then
					...
					do ivpsa = 1,nvpsa ! Clara 
						if (tag_mat(im).eq.tagvpsa(ivpsa)) then ! Clara 
							...
							a1n1(i,j,ie) =  0.577350269 !0.00001773             !MCNAP vpsa
							a1n2(i,j,ie) =  0.577350269 !0.00001773             !MCNAP vpsa
							a1n3(i,j,ie) =  0.577350269 !0.00001773             !MCNAP vpsa
							...
							u7n(i,j,ie) =valvpsa(which_vpsa,2)     !MCNAP vpsa
...
! WORK IN PROGRESS
	  ...
	  if (nmaps.gt.0) then														 ! Clara&Marco Pre-Stress 9May05
		do im = 1,nm															 ! Clara&Marco Pre-Stress 9May05
		
		if (type_mat(im).eq.2) then												 ! Clara&Marco Pre-Stress 9May05
			do k = 1,nelem_ebe													 ! Clara&Marco Pre-Stress 9May05
				ie = ielem_ebe(k)
				which_maps = 0													 ! Clara&Marco Pre-Stress 9May05
				do imaps = 1,nmaps												 ! Clara&Marco Pre-Stress 9May05
			         !-------------------------------------------------------------------------
					 !THIS OPTION IS USELESS: GEOSTATIC STRESSES COMPUTED WITH "k0" or "Jaky"
					 !-------------------------------------------------------------------------
					if (fmaps(imaps).le.3) then									 ! Clara&Marco Pre-Stress 9May05

						if (tag_mat(im).eq.tagmaps(imaps)) then					  ! Clara&Marco Pre-Stress 9May05
							which_maps = imaps									  ! Clara&Marco Pre-Stress 9May05

							call make_pre_stress(prop_mat(imaps,1),prop_mat(imaps,2),prop_mat(imaps,3),&    ! Clara&Marco Pre-Stress 9May05
					 					  valmaps(imaps,1),fmaps(imaps),&                                   ! Clara&Marco Pre-Stress 9May05
					 					  sxx_ps,syy_ps,sxy_ps,szz_ps,&                                     ! Clara&Marco Pre-Stress 9May05
					 					  nn,ie,nelem_ebe,&                                                 ! Clara&Marco Pre-Stress 9May05
					 				   	  nnt,xs,ys,&                                                       ! Clara&Marco Pre-Stress 9May05
					 					  cs_nnz,cs,node_index,&											! Clara&Marco Pre-Stress 9May05
										  N_update_el_az,update_index_el_az,&								! Clara&Marco Pre-Stress 9May05
								          file_out,nsnapps,itersnapps,&										! Clara&Marco Pre-Stress 9May05
						                  sxx_ps_read,syy_ps_read,sxy_ps_read,szz_ps_read,&					! Clara&Marco Pre-Stress 9May05
										  !vistnxx_ps_read,vistnyy_ps_read,vistnxy_ps_read,&				! Clara&Marco Pre-Stress 9May05
										  nprocs)															! Clara&Marco Pre-Stress 9May05
						endif													! Clara&Marco Pre-Stress 9May05					 
                
					endif
					!-------------------------------------------------------------------------														! Clara&Marco Pre-Stress 9May05
 				enddo !imaps													! Clara&Marco Pre-Stress 9May05
           
			   enddo ! k														! Clara&Marco Pre-Stress 9May05
			endif																! Clara&Marco Pre-Stress 9May05

 
		enddo ! im																! Clara&Marco Pre-Stress 9May05

		do imaps = 1,nmaps													     ! Clara&Marco Pre-Stress 9May05
		            !-------------------------------------------------------------------------
					!PRE-STRESSES ARE COMPUTED WITH A PREVIOUS ANALYSIS (SNAPPRES)  !!!
				    !-------------------------------------------------------------------------												
																
			if (fmaps(imaps).eq.4) then											! Clara&Marco Pre-Stress 9May05
					write(*,'(A)')'FIRST STEP1a'								! Clara&Marco Pre-Stress 9May05

					    call read_initial_condition(nnt,file_out,nsnapps,itersnapps,&		! Clara&Marco Pre-Stress 9May05
								 u1_read,u0_read,&											! Clara&Marco Pre-Stress 9May05
								 Fe_read,Fk_read,&											! Clara&Marco Pre-Stress 9May05
								 vivel_read,&												! Clara&Marco Pre-Stress 9May05
								 yield_read,&												! Marco 4Apr2005 - Output yield
								 vnucl_read,&												! Marco 4Apr2005 - Output yield
								 vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read,&	! Clara 23/04/05
								 nprocs)													! Clara&Marco Pre-Stress 9May05
					

				     write(*,'(A)')'FIRST STEP1b'											! Clara&Marco Pre-Stress 9May05
			endif																			! Clara&Marco Pre-Stress 9May05
		          !-------------------------------------------------------------------------		
			...
			if (fmaps(imaps).eq.5) then															! Clara SNAPPRES SAND
					
					    call read_initial_condition_sand(nnt,file_out,nsnapps,itersnapps,&		! Clara SNAPPRES SAND
								 u1_read,u0_read,&											    ! Clara SNAPPRES SAND
								 Fe_read,Fk_read,&										    	! Clara SNAPPRES SAND
								 vivel_read,&											    	! Clara SNAPPRES SAND
								 yield_read,&											    	! Clara SNAPPRES SAND
								 vnucl_read,&											    	! Clara SNAPPRES SAND
								 vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read,&	    ! Clara SNAPPRES SAND
								 nprocs,&													    ! Clara SNAPPRES SAND
					             a1n1_read,a1n2_read,a1n3_read,a1n4_read,a1n5_read,a1n6_read,&  ! Clara SNAPPRES SAND
								 epn1_read,epn2_read,epn3_read,epn4_read,epn5_read,epn6_read,&  ! Clara SNAPPRES SAND
								 z14n_read,u7n_read,densit_read)                                ! Clara SNAPPRES SAND
				    
			endif																		    	! Clara SNAPPRES SAND
		          !-------------------------------------------------------------------------		
		
		
		enddo																			    	! Clara SNAPPRES SAND
	  ...
	  write(*,'(A)')'FIRST STEP2'
         ...
         if (nmaps.gt.0) then									! M Mexico _A_
			do imaps=1,nmaps									! M Mexico _A_
				if (fmaps(nmaps).eq.4) then						! M Mexico _A_
					...
					! si dovrebbe poter pre-stressare solo alcuni materiali e non tutti!!!
						...
						u1_az(iaz)  = u1_read(iaz)				! M Mexico _A_
						u0_az(iaz)  = u0_read(iaz)				
						u_1_az(iaz) = u0_read(iaz)
						fe_az(iaz)  = Fe_read(iaz)				! M Mexico _A_
						fk_az(iaz)  = Fk_read(iaz)				! M Mexico _A_
						
						vivel_az(iaz)  = vivel_read(iaz)		! M Mexico _A_
						yield_az(iaz)  = yield_read(iaz)		! Marco 4Apr2005 - Output yield
						vnucl_az(iaz)  = vnucl_read(iaz)		! Marco 4Apr2005 - Output yield
						...
						vistn4_0_az(iaz)  = vistn4_0_read(iaz)	! Clara 23/04/05
			...
			if (fmaps(nmaps).eq.5) then							! Clara SNAPPRES SAND
					
					do iaz = 0,N_update_el_az -1				! Clara SNAPPRES SAND    
					! 
					! I don't like this solution!!! Not general: 
					! It should be modified ASAP!!!
					! si dovrebbe poter pre-stressare solo alcuni materiali e non tutti!!!

						u1_az(iaz)  = u1_read(iaz)				! Clara SNAPPRES SAND
						u0_az(iaz) = u0_read(iaz)				! Clara SNAPPRES SAND
						u_1_az(iaz) = u0_read(iaz)				! Clara SNAPPRES SAND
						fe_az(iaz)  = Fe_read(iaz)				! Clara SNAPPRES SAND
						fk_az(iaz)  = Fk_read(iaz)				! Clara SNAPPRES SAND
						
						vivel_az(iaz)  = vivel_read(iaz)		! Clara SNAPPRES SAND
						yield_az(iaz)  = yield_read(iaz)		! Clara SNAPPRES SAND
						vnucl_az(iaz)  = vnucl_read(iaz)		! Clara SNAPPRES SAND
						
						vistn1_0_az(iaz)  = vistn1_0_read(iaz)	! Clara SNAPPRES SAND
						vistn2_0_az(iaz)  = vistn2_0_read(iaz)	! Clara SNAPPRES SAND
						vistn3_0_az(iaz)  = vistn3_0_read(iaz)	! Clara SNAPPRES SAND
						vistn4_0_az(iaz)  = vistn4_0_read(iaz)	! Clara SNAPPRES SAND

						a1n1_az(iaz) = a1n1_read(iaz) ! Clara SNAPPRES SAND        
						a1n2_az(iaz) = a1n2_read(iaz) ! Clara SNAPPRES SAND        
						a1n3_az(iaz) = a1n3_read(iaz) ! Clara SNAPPRES SAND        
						a1n4_az(iaz) = a1n4_read(iaz) ! Clara SNAPPRES SAND        
						a1n5_az(iaz) = a1n5_read(iaz) ! Clara SNAPPRES SAND        
						a1n6_az(iaz) = a1n6_read(iaz) ! Clara SNAPPRES SAND        
						epn1_az(iaz) = epn1_read(iaz) ! Clara SNAPPRES SAND        
						epn2_az(iaz) = epn2_read(iaz) ! Clara SNAPPRES SAND        
						epn3_az(iaz) = epn3_read(iaz) ! Clara SNAPPRES SAND        
						epn4_az(iaz) = epn4_read(iaz) ! Clara SNAPPRES SAND        
						epn5_az(iaz) = epn5_read(iaz) ! Clara SNAPPRES SAND        
						epn6_az(iaz) = epn6_read(iaz) ! Clara SNAPPRES SAND        
						z14n_az(iaz) = z14n_read(iaz) ! Clara SNAPPRES SAND        
						u7n_az(iaz)  = u7n_read(iaz)   ! Clara SNAPPRES SAND       
						densit_az(iaz)  = densit_read(iaz)   ! Clara SNAPPRES SAND 
      ...
      if (nmaps.gt.0) then										! M Mexico _A_
		do imaps=1,nmaps										! M Mexico _A_
	  		...
	  		do i = 1,nn											! M Mexico _A_
	  			do j = 1,nn										! M Mexico _A_
	  				do ie = 1,nelem_ebe							! M Mexico _A_
						...
						yield_el(i,j,ie) = yield_az(iaz)		! Marco 4Apr2005 - Output yield
						vnucl_el(i,j,ie) = vnucl_az(iaz)		! Marco 4Apr2005 - Output yield
                        ...
                        vistn4_0(i,j,ie) = vistn4_0_az(iaz)		! Clara 23/04/05
			...
			if (fmaps(nmaps).eq.5) then							 ! Clara SNAPPRES SAND
	  		do i = 1,nn											 ! Clara SNAPPRES SAND
	  			do j = 1,nn										 ! Clara SNAPPRES SAND
	  				do ie = 1,nelem_ebe							 ! Clara SNAPPRES SAND
      
	  					is = nn*(j -1) +i						 ! Clara SNAPPRES SAND
	  					in = cs(cs(ie -1) + is)					 ! Clara SNAPPRES SAND
	  			
	  					id1 = node_index(in)					 ! Clara SNAPPRES SAND
	  					iaz = update_index_el_az(id1 -1)		 ! Clara SNAPPRES SAND

						vivel_el(i,j,ie) = vivel_az(iaz)		 ! Clara SNAPPRES SAND
						yield_el(i,j,ie) = yield_az(iaz)		 ! Clara SNAPPRES SAND
						vnucl_el(i,j,ie) = vnucl_az(iaz)		 ! Clara SNAPPRES SAND

	  					vistn1_0(i,j,ie) = vistn1_0_az(iaz)	    ! Clara SNAPPRES SAND
	  					vistn2_0(i,j,ie) = vistn2_0_az(iaz)		! Clara SNAPPRES SAND_
	  					vistn3_0(i,j,ie) = vistn3_0_az(iaz)		! Clara SNAPPRES SAND
                        vistn4_0(i,j,ie) = vistn4_0_az(iaz)		! Clara SNAPPRES SAND

						
						a1n1(i,j,ie)   = a1n1_az(iaz)   ! Clara SNAPPRES SAND
						a1n2(i,j,ie)   = a1n2_az(iaz)   ! Clara SNAPPRES SAND
						a1n3(i,j,ie)   = a1n3_az(iaz)   ! Clara SNAPPRES SAND
						a1n4(i,j,ie)   = a1n4_az(iaz)   ! Clara SNAPPRES SAND
						a1n5(i,j,ie)   = a1n5_az(iaz)   ! Clara SNAPPRES SAND
						a1n6(i,j,ie)   = a1n6_az(iaz)   ! Clara SNAPPRES SAND
						epn1(i,j,ie)   = epn1_az(iaz)   ! Clara SNAPPRES SAND
						epn2(i,j,ie)   = epn2_az(iaz)   ! Clara SNAPPRES SAND
						epn3(i,j,ie)   = epn3_az(iaz)   ! Clara SNAPPRES SAND
						epn4(i,j,ie)   = epn4_az(iaz)   ! Clara SNAPPRES SAND
						epn5(i,j,ie)   = epn5_az(iaz)   ! Clara SNAPPRES SAND
						epn6(i,j,ie)   = epn6_az(iaz)   ! Clara SNAPPRES SAND
						z14n(i,j,ie)   = z14n_az(iaz)   ! Clara SNAPPRES SAND
						u7n(i,j,ie)    = u7n_az(iaz)    ! Clara SNAPPRES SAND
						densit(i,j,ie) = densit_az(iaz) ! Clara SNAPPRES SAND
      ...
      !-------------------------------------------------------- 
		...
		if (nmaps.gt.0) then ! M Mexico _A_
			do imaps=1,nmaps ! M Mexico _A_
				if (fmaps(nmaps).eq.4.or.fmaps(nmaps).eq.5) then ! M Mexico _A_   ! Clara SNAPPRES SAND

				    do iaz = 0,N_update_el_az -1
						u2_star(iaz) = u1_az(iaz) ! M Mexico _A_
						!u1_az(iaz) = u0_az(iaz) ! M Mexico _A_
...
! ********************************************************* 
	...
	do iaz = 0,N_update_el_az -1
      vel_az(iaz) = ( u1_az(iaz) - u_1_az(iaz) ) / dt ! Marco 16Sep05
	  acc_az(iaz) = ( u1_az(iaz) + 2 * u0_az(iaz) - u_1_az(iaz) ) / dt2 ! Marco 16Sep05
	enddo
...
! *********************************************************         
      ...
      if ((nsnap.gt.0).and.(isnap.le.nsnap)) then
         if (its.ge.itersnap(isnap)) then
			...
			call write_bin_file(file_out_acceleration,isnap,myid,& ! Marco 16Sep05
                                N_update_el_az,out_index_el_az,vel_az) ! Marco 16Sep05

			call write_bin_file(file_out_velocity,isnap,myid,& ! Marco 16Sep05
                                N_update_el_az,out_index_el_az,acc_az) ! Marco 16Sep05

			call write_bin_file(file_outV,isnap,myid,& ! Clara 
                                N_update_el_az,out_index_el_az,vivel_az) ! Clara

			call write_bin_file(file_outY,isnap,myid,& ! Marco 4Apr2005 - Output yield 
                                N_update_el_az,out_index_el_az,yield_az) ! Marco 4Apr2005 - Output yield
								
			call write_bin_file(file_outF,isnap,myid,& ! Marco 4Apr2005 - Output yield 
                                N_update_el_az,out_index_el_az,vnucl_az) ! Marco 4Apr2005 - Output yield 
            ...
            call write_bin_file(file_outT4,isnap,myid,& ! Clara 23/04/05
                                N_update_el_az,out_index_el_az,vistn4_0_az) ! Clara 23/04/05
	  ...
	  if ((nsnapps.gt.0).and.(isnapps.le.nsnapps)) then
		if (its.ge.itersnapps(isnapps)) then
			...
			call write_bin_file(file_outS4,isnapps,myid,& ! Clara 
                                N_update_el_az,out_index_el_az,szz_az) ! Clara
			isnapps = isnapps + 1
...
write(*,'(A)')'ALL STEPS'
	...
	enddo ! Clara 
      ...
      do its = 1,nts
	     ...
	     if ( (its*dt).ge.time_change_sampling) then ! Marco 25Sep05
			ndt_monitor = ndt_change_monitor		 ! Marco 25Sep05
		 endif										 ! Marco 25Sep05
      ...
      !-------------------------------------------------------- 
		...
		do iaz = 0,N_update_el_az -1
			...
			yield_az_1(iaz) = yield_az(iaz)			! Marco 4Apr2005 - Output yield
			vnucl_az_1(iaz) = vnucl_az(iaz)			! Marco 4Apr2005 - Output yield
            ...
            vistn4_0_az_1(iaz) = vistn4_0_az(iaz)	! Clara 23/04/05
      ...
      !-------------------------------------------------------- 
                  ...
                  do k = 1,nelem_ebe
                     ...
                     if (cs(cs(ie -1) +0).eq.tag_mat(im)) then
                        ...
                        call make_internal_force(lambda,mu,rho,&
                                      ...
                                      sxx_el,syy_el,sxy_el,szz_el,fx_el,fy_el,&
                                      check_node_sism,check_dist_node_sism,&
									  ...
									  pcc_el,& !Cam_Clay 
									  vistn1_0,vistn2_0,vistn3_0,vistn4_0,& ! Clara 23/04/05 
									  ...
									  sum_end_nloc_vpcl,&  ! M NON LOCAL 02Mar05
									  connectivity_nloc_vpcl,weigth_nloc_vpcl,&  ! M NON LOCAL 02Mar05
									  yield1_az_vpcl,yield2_az_vpcl,nodal_counter,check_in_vpcl,& ! M NON LOCAL 02Mar05
									  iq_nloc_vpcl,ip_nloc_vpcl,&
									  integral_nloc_vpcl,&
									  begin_nloc_vpsa,end_nloc_vpsa,& ! M NON LOCAL 02Mar05
									  sum_end_nloc_vpsa,&  ! M NON LOCAL 02Mar05
									  connectivity_nloc_vpsa,weigth_nloc_vpsa,&  ! M NON LOCAL 02Mar05
									  yield1_az_vpsa,yield2_az_vpsa,check_in_vpsa,& ! M NON LOCAL 02Mar05
									  iq_nloc_vpsa,ip_nloc_vpsa,&
									  integral_nloc_vpsa,&
									  yield_el,vnucl_el) ! Marco 4Apr2005 - Output yield
                        ...
                        do j = 1,nn
                           do i = 1,nn
                              ...
                              if (node_ind_ebe(in).ne.0) then
								 ...
								 szz_az(iaz) = szz_az(iaz) + szz_el(i,j)! Clara
								 duxdx_az(iaz) = duxdx_az(iaz) + duxdx_el(i,j) ! Marco 10.02.2005 "Strain Output" 
								 duydy_az(iaz) = duydy_az(iaz) + duydy_el(i,j) ! Marco 10.02.2005 "Strain Output" 
								 duxdy_az(iaz) = duxdy_az(iaz) + (duxdy_el(i,j)+duydx_el(i,j)) ! Marco 10.02.2005 "Strain Output"  !Clara output 28.11.06
								 vivel_az(iaz) = vivel_az(iaz) + vivel_el(i,j,ie)
								 yield_az(iaz) = yield_az(iaz) + yield_el(i,j,ie) ! Marco 4Apr2005 - Output yield
                                 vnucl_az(iaz) = vnucl_az(iaz) + vnucl_el(i,j,ie) ! Marco 4Apr2005 - Output yield
								 ...
								 vistn4_0_az(iaz) = vistn4_0_az(iaz) + vistn4_0(i,j,ie) ! Clara 23/04/05

								 if (which_vpsa.ne.0) then
									a1n1_az(iaz) = a1n1_az(iaz) + a1n1(i,j,ie) ! Clara SNAPPRES SAND
									a1n2_az(iaz) = a1n2_az(iaz) + a1n2(i,j,ie) ! Clara SNAPPRES SAND
									a1n3_az(iaz) = a1n3_az(iaz) + a1n3(i,j,ie) ! Clara SNAPPRES SAND
									a1n4_az(iaz) = a1n4_az(iaz) + a1n4(i,j,ie) ! Clara SNAPPRES SAND
									a1n5_az(iaz) = a1n5_az(iaz) + a1n5(i,j,ie) ! Clara SNAPPRES SAND
									a1n6_az(iaz) = a1n6_az(iaz) + a1n6(i,j,ie) ! Clara SNAPPRES SAND
									epn1_az(iaz) = epn1_az(iaz) + epn1(i,j,ie) ! Clara SNAPPRES SAND
									epn2_az(iaz) = epn2_az(iaz) + epn2(i,j,ie) ! Clara SNAPPRES SAND
									epn3_az(iaz) = epn3_az(iaz) + epn3(i,j,ie) ! Clara SNAPPRES SAND
									epn4_az(iaz) = epn4_az(iaz) + epn4(i,j,ie) ! Clara SNAPPRES SAND
									epn5_az(iaz) = epn5_az(iaz) + epn5(i,j,ie) ! Clara SNAPPRES SAND
									epn6_az(iaz) = epn6_az(iaz) + epn6(i,j,ie) ! Clara SNAPPRES SAND
									z14n_az(iaz) = z14n_az(iaz) + z14n(i,j,ie) ! Clara SNAPPRES SAND
									u7n_az(iaz) = u7n_az(iaz) + u7n(i,j,ie) ! Clara SNAPPRES SAND
									densit_az(iaz) = densit_az(iaz) + densit(i,j,ie) ! Clara SNAPPRES SAND
								endif
		 ...
		 do ivpcl = 1,nvpcl										! M NON LOCAL 02Mar05
			if (valvpcl(ivpcl,7).gt.0) then						! M NON LOCAL 02Mar05
				do in = 1,nnt									! M NON LOCAL 02Mar05
					!yield2_az(ivpcl,in) = yield_new(in) / nodal_counter(in)
					!yield_new(in) = 0.0d0		
					yield2_az_vpcl(ivpcl,in) = yield1_az_vpcl(ivpcl,in) !/ nodal_counter(in)	! M NON LOCAL 02Mar05
					!write(54,*) in,yield1_az(ivpcl,in)
					yield1_az_vpcl(ivpcl,in) = 0.0d0					! M NON LOCAL 02Mar05
					check_in_vpcl(in) = 0		
				enddo											! M NON LOCAL 02Mar05
			endif												! M NON LOCAL 02Mar05
        ...
        do ivpsa = 1,nvpsa										! M NON LOCAL 02Mar05
			if (valvpsa(ivpsa,3).gt.0) then						! M NON LOCAL 02Mar05
				do in = 1,nnt									! M NON LOCAL 02Mar05
					!yield2_az(ivpcl,in) = yield_new(in) / nodal_counter(in)
					!yield_new(in) = 0.0d0		
					yield2_az_vpsa(ivpsa,in) = yield1_az_vpsa(ivpsa,in) !/ nodal_counter(in)	! M NON LOCAL 02Mar05
					!write(54,*) in,yield1_az(ivpcl,in)
					yield1_az_vpsa(ivpsa,in) = 0.0d0					! M NON LOCAL 02Mar05
					check_in_vpsa(in) = 0		
				enddo											! M NON LOCAL 02Mar05
		 ...
		 do in = 1,nnt ! Clara 
			...
			yield_az(iaz) = yield_az(iaz) / nodal_counter(in) ! Marco 4Apr2005 - Output yield
			vnucl_az(iaz) = vnucl_az(iaz) / nodal_counter(in) ! Marco 4Apr2005 - Output yield
			...
			vistn4_0_az(iaz) = vistn4_0_az(iaz) / nodal_counter(in) ! Clara 23/04/05
			...
			szz_az(iaz) = szz_az(iaz) / nodal_counter(in) ! Clara
			duxdx_az(iaz) = duxdx_az(iaz) / nodal_counter(in) ! Marco 10.02.2005 "Strain Output"
			duydy_az(iaz) = duydy_az(iaz) / nodal_counter(in) ! Marco 10.02.2005 "Strain Output"
			duxdy_az(iaz) = duxdy_az(iaz) / nodal_counter(in) ! Marco 10.02.2005 "Strain Output"

            if (which_vpsa.ne.0) then
				a1n1_az(iaz)   = a1n1_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				a1n2_az(iaz)   = a1n2_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				a1n3_az(iaz)   = a1n3_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				a1n4_az(iaz)   = a1n4_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				a1n5_az(iaz)   = a1n5_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				a1n6_az(iaz)   = a1n6_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn1_az(iaz)   = epn1_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn2_az(iaz)   = epn2_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn3_az(iaz)   = epn3_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn4_az(iaz)   = epn4_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn5_az(iaz)   = epn5_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				epn6_az(iaz)   = epn6_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				z14n_az(iaz)   = z14n_az(iaz) / nodal_counter(in)  ! Clara SNAPPRES SAND  
				u7n_az(iaz)    = u7n_az(iaz)  / nodal_counter(in)  ! Clara SNAPPRES SAND  
				densit_az(iaz) = densit_az(iaz) / nodal_counter(in) ! Clara SNAPPRES SAND
			endif 
...
! Fourth edge
                        ...
                        call make_absorbing_force(lambda,mu,rho,&
								  nmatg,valmatg,which_matg,tagmatg,typematg,& ! Mexico Paco grad_lin 29.11.2004
								  tag_mat(im),&
								  nn,ct,ww,dd,&
                                  dxdx_el,dxdy_el,dydx_el,dydy_el,&
...
!        End Dirichlet b.c.
		 ...
		 if (int(real(its)/ndt_monitor).eq.(real(its)/ndt_monitor)) then
			...
			do i = 1,nmonit												! Clara 
               ...
               if (node_index(in).ne.0) then							! Clara 
                  id1 = node_index(in)									! Clara 
                  iaz = update_index_el_az(id1 -1)						! Clara 
                  write(unit_monitor,'(5E16.8)') &						! Clara 
                       tt1,sxx_az(iaz),syy_az(iaz),sxy_az(iaz),&		! Clara
					   szz_az(iaz)										! Clara 
               endif													! Clara 
            enddo														! Clara 

																		! M "me&you" paper 09.02.2005
            do i = 1,nmonit												! M "me&you" paper 09.02.2005
               unit_monitor = 20000 + i									! M "me&you" paper 09.02.2005
               in = node_m(i)											! M "me&you" paper 09.02.2005
               if (node_index(in).ne.0) then							! M "me&you" paper 09.02.2005
                  id1 = node_index(in)									! M "me&you" paper 09.02.2005
                  id2 = node_index(in) +nnd								! M "me&you" paper 09.02.2005
                  iaz = update_index_el_az(id1 -1)						! M "me&you" paper 09.02.2005
                  jaz = update_index_el_az(id2 -1)						! M "me&you" paper 09.02.2005
                  write(unit_monitor,'(3E16.8)') &						! M "me&you" paper 09.02.2005
                       tt1,fk_az(iaz),fk_az(jaz)						! M "me&you" paper 09.02.2005
               endif													! M "me&you" paper 09.02.2005
            enddo														! M "me&you" paper 09.02.2005
																		! M "me&you" paper 09.02.2005
			...
			do i = 1,nmonit												! Marco 10.02.2005 "Strain Output" 
               ...
               in = node_m(i)											! Marco 10.02.2005 "Strain Output" 
               if (node_index(in).ne.0) then							! Marco 10.02.2005 "Strain Output" 
                  id1 = node_index(in)									! Marco 10.02.2005 "Strain Output" 
                  iaz = update_index_el_az(id1 -1)						! Marco 10.02.2005 "Strain Output" 
                  write(unit_monitor,'(4E16.8)') &						! Marco 10.02.2005 "Strain Output" 
                       tt1,duxdx_az(iaz),duydy_az(iaz),duxdy_az(iaz)	! Marco 10.02.2005 "Strain Output" 
               endif													! Marco 10.02.2005 "Strain Output" 
            enddo														! Marco 10.02.2005 "Strain Output"
			...
			do i = 1,nmonit												! Marco 4Apr2005 - Output yield
               ...
               in = node_m(i)											! Marco 4Apr2005 - Output yield 
               if (node_index(in).ne.0) then							! Marco 4Apr2005 - Output yield
                  id1 = node_index(in)									! Marco 4Apr2005 - Output yield
                  iaz = update_index_el_az(id1 -1)						! Marco 4Apr2005 - Output yield 
                  write(unit_monitor,'(2E16.8)') &						! Marco 4Apr2005 - Output yield 
                       tt1,yield_az(iaz)								! Marco 4Apr2005 - Output yield
               endif													! Marco 4Apr2005 - Output yield 
            enddo														! Marco 4Apr2005 - Output yield
			...
			do i = 1,nmonit												! Marco 4Apr2005 - Output yield
               ...
               if (node_index(in).ne.0) then							! Marco 4Apr2005 - Output yield
                  ...
                  iaz = update_index_el_az(id1 -1)						! Marco 4Apr2005 - Output yield 
                  write(unit_monitor,'(2E16.8)') &						! Marco 4Apr2005 - Output yield 
                       tt1,vnucl_az(iaz)								! Marco 4Apr2005 - Output yield
               endif													! Marco 4Apr2005 - Output yield 
            enddo														! Marco 4Apr2005 - Output yield
            ...
            do i = 1,nmonit												! Marco 24.04.2005
               ...
               in = node_m(i)											! Marco 24.04.2005
               if (node_index(in).ne.0) then							! Marco 24.04.2005
                  id1 = node_index(in)									! Marco 24.04.2005
                  id2 = node_index(in) +nnd								! Marco 24.04.2005
                  iaz = update_index_el_az(id1 -1)						! Marco 24.04.2005
                  jaz = update_index_el_az(id2 -1)						! Marco 24.04.2005
                  write(unit_monitor,'(3E16.8)') &						! Marco 24.04.2005
                       tt1,fe_az(iaz),fe_az(jaz)						! Marco 24.04.2005
               endif													! Marco 24.04.2005
            enddo														! Marco 24.04.2005
																		! Marco 24.04.2005
		...
		do iaz = 0,N_update_el_az -1
			vel_az(iaz) = ( u2_az(iaz) - u0_az(iaz) ) / dt ! Marco 16Sep05
			acc_az(iaz) = ( u2_az(iaz) + 2 * u1_az(iaz) - u0_az(iaz) ) / dt2 ! Marco 16Sep05
		enddo
...
! *********************************************************         
         ...
         if ((nsnap.gt.0).and.(isnap.le.nsnap)) then
            if (its.ge.itersnap(isnap)) then
				...
				!INTRODUCE OUTPUT IN VELOCITY AND ACCELERATION!!!TONIGHT
				! Marco 16Sep05
	  ...
	  if ((nsnapps.gt.0).and.(isnapps.le.nsnapps)) then
		if (its.ge.itersnapps(isnapps)) then
			...
			call write_bin_file_new(file_out_u1,isnapps,myid,&					! M Mexico _A_ 
                                N_update_el_az,out_index_el_az,u2_az)			! M Mexico _A_
		    ...
		    call write_bin_file_new(file_out_yield,isnapps,myid,&				! Marco 4Apr2005 - Output yield  USELESS!!!
                                N_update_el_az,out_index_el_az,yield_az_1)		! Marco 4Apr2005 - Output yield  USELESS!!!
			call write_bin_file_new(file_out_vnucl,isnapps,myid,&				! Marco 4Apr2005 - Output yield  USELESS!!!
                                N_update_el_az,out_index_el_az,vnucl_az_1)		! Marco 4Apr2005 - Output yield  USELESS!!!
            ...
            call write_bin_file_new(file_out_vistn4_0,isnapps,myid,&			! Clara 23/04/05 
                                N_update_el_az,out_index_el_az,vistn4_0_az_1)	! Clara 23/04/05



			call write_bin_file_new(file_out_a1n1,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n1_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_a1n2,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n2_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_a1n3,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n3_az)			! Clara SNAPPRES SAND
	        call write_bin_file_new(file_out_a1n4,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n4_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_a1n5,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n5_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_a1n6,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,a1n6_az)			! Clara SNAPPRES SAND
			
			call write_bin_file_new(file_out_epn1,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn1_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_epn2,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn2_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_epn3,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn3_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_epn4,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn4_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_epn5,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn5_az)			! Clara SNAPPRES SAND
			call write_bin_file_new(file_out_epn6,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,epn6_az)			! Clara SNAPPRES SAND
			
			call write_bin_file_new(file_out_z14n,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,z14n_az)			! Clara SNAPPRES SAND

			call write_bin_file_new(file_out_u7n,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,u7n_az)			! Clara SNAPPRES SAND

			call write_bin_file_new(file_out_densit,isnapps,myid,&				! Clara SNAPPRES SAND
                                N_update_el_az,out_index_el_az,densit_az)			! Clara SNAPPRES SAND
...
!     
      ...
      integer*4 :: i
      real*8 :: val,PI,tt_t0,t0,t1,v0,v1,tt_t1,tt_t2,tt_t3
	  ...
	  elseif (func_type(n).eq.12) then
        ...
        ! func_data(func_indx(n)) = a3-a1
		! func_data(func_indx(n) +1) = a2
	    ! func_data(func_indx(n) +2) = t0
		! Parameters refer to the following function:
		! y= (a3-a1).*(1./(1+exp(-a2.*(tt_t0))));
	  ...
	  ! 26 - Amp.*sin(2*pi*f.*(t-t0))
	  !
	  ! N cycles of SIN function
      ! 
      ! parameters:
	  ! * f  = func_data(func_indx(n))
	  ! * t0 = func_data(func_indx(n) +1)
	  ! * n  = func_data(func_indx(n) +2)
	  ! * amp  = func_data(func_indx(n) +3)
	  !
      !  /\  amp.*sin(2*pi*f.*(t-t0))
      !  |                                                                                                
      !  |         ***                     ***     
	  ...
	  elseif (func_type(n).eq.26) then
	    ...
	    elseif ((tt_t0 - func_data(func_indx(n) +2) * 1 / func_data(func_indx(n)) ).ge.0) then
			...
			PI = 4.0d0 * datan(1.0d0)
			val = func_data(func_indx(n) +3) * sin (2*PI*func_data(func_indx(n)) * (tt_t0))
		
		endif	  
				
		
	 	  ! 6 - Ramp function + Amp.*sin(2*pi*f.*(t-t0)):
	  !
	  !
	  !   /\ F(t)
	  !   |
      ...
      !
	  ! parameters:
	  ! * f  = func_data(func_indx(n +4))
	  ! * t0 = func_data(func_indx(n) +5)
	  ! * n  = func_data(func_indx(n) +6)
	  ! * amp  = func_data(func_indx(n) +7)
	  !

      elseif (func_type(n).eq.27) then

         tt_t0 = t - func_data(func_indx(n) +2)
		 tt_t1 = t - func_data(func_indx(n) +5)

		 if (tt_t0.le.0) then

			val = func_data(func_indx(n))

	     elseif (t.le.func_data(func_indx(n) +3)) then
         ...
         elseif (t.le.(func_data(func_indx(n) +3) + func_data(func_indx(n) +6) * 1 / func_data(func_indx(n) +4))) then

			PI = 4.0d0 * datan(1.0d0)
			val =  func_data(func_indx(n) + 1) + &
				  func_data(func_indx(n) +7) * sin (2*PI*func_data(func_indx(n) +4) * (tt_t1))
      ...
      !
	  ! parameters:
	  ! * S0 = func_data(func_indx(n))
	  ! * S1 = func_data(func_indx(n +1))
	  ! * t0 = func_data(func_indx(n +2))
	  ! * t1 = func_data(func_indx(n +3))
      !
	  ! * f2  = func_data(func_indx(n +4))
	  ! * t2 = func_data(func_indx(n) +5)
	  ! * n2  = func_data(func_indx(n) +6)
	  ! * amp2  = func_data(func_indx(n) +7)
	  !
	  ! * f3  = func_data(func_indx(n +8))
	  ! * t3 = func_data(func_indx(n) +9)
	  ! * n3  = func_data(func_indx(n) +10)
	  ! * amp3  = func_data(func_indx(n) +11)
	  !

      elseif (func_type(n).eq.28) then

         tt_t0 = t - func_data(func_indx(n) +2)
		 tt_t2 = t - func_data(func_indx(n) +5)
		 tt_t3 = t - func_data(func_indx(n) +9)


		 if (t.le.func_data(func_indx(n) +2)) then
	     ...
	     elseif (t.le.func_data(func_indx(n) +5)) then

			val = func_data(func_indx(n) +1)


        elseif (t.le.(func_data(func_indx(n) +5) + func_data(func_indx(n) +6) * 1 / func_data(func_indx(n) +4))) then
	     !elseif (t.le.( func_data(func_indx(n) +5) + func_data(func_indx(n) +6)*1/func_data(func_indx(n )+4) ) ) then

			  PI = 4.0d0 * datan(1.0d0)
		      val = func_data(func_indx(n) +1) + &
			        func_data(func_indx(n) +7) * sin( 2*pi*func_data(func_indx(n) +4)*(tt_t2) )

         elseif (t.le.func_data(func_indx(n) +9)) then
			  
			  val = func_data(func_indx(n) +1)
        
		elseif (t.le.(func_data(func_indx(n) +9) + func_data(func_indx(n) +10) * 1 / func_data(func_indx(n) +8))) then
         !elseif (t.le.( func_data(func_indx(n) +9) + func_data(func_indx(n) +10)*1/func_data(func_indx(n) +8) ) ) then

			  PI = 4.0d0 * datan(1.0d0)
		      val = func_data(func_indx(n) +1) + &
			        func_data(func_indx(n) +11) * sin(2 *PI*func_data(func_indx(n) +8)*(tt_t3) )
			  !write(*,*)t,val
	  ...
	  elseif (func_type(n).eq.29) then
	    ...
	    elseif ((tt_t0 - func_data(func_indx(n) +2) * 1 / func_data(func_indx(n)) ).ge.0) then
			...
			PI = 4.0d0 * datan(1.0d0)
			val = func_data(func_indx(n) +3) * sin (2*PI*func_data(func_indx(n)) * (tt_t0))**2
      ...
      elseif (func_type(n).eq.30) then

         tt_t0 = t - func_data(func_indx(n) +2)

		 if (func_data(func_indx(n) + 3).lt.t) then

			val = func_data(func_indx(n) + 1)*2
...
!     *****************************************************************
      ...
      subroutine read_dime_header(file_head,nmonitors,nmonitorsl,nsnapshots,nsnapsprestress) ! Marco
...
!
      ...
      nmonitors = 0
	  nmonitorsl = 0 ! Marco - MONITOR LINE - 14Feb2005 
      nsnapshots = 0
      ...
      do
		 ...
		 elseif (keyword(1:7).eq.'MONITOL') then
            nmonitorsl = nmonitorsl + 1	! Marco - MONITOR LINE - 14Feb2005 
...
!
	  ...
	  real*8, dimension(*) :: x1_monitorl,y1_monitorl ! Marco - MONITOR LINE - 14Feb2005
	  real*8, dimension(*) :: x2_monitorl,y2_monitorl ! Marco - MONITOR LINE - 14Feb2005 
      ...
      integer*4 :: i,j,im,is
	  integer*4 :: iml ! Marco - MONITOR LINE - 14Feb2005 
	  integer*4 :: ips ! Marco
      real*8 :: val
      real*8 :: factor_scale_mesh	! Marco - SCALE MESH 11 May 2005
      ...
      ndt_monitor = 1
	  ndt_change_monitor = 1 !Marco 25Sep05
	  time_change_sampling = 0 !Marco 25Sep05
      im = 0
	  iml = 0  ! Marco - MONITOR LINE - 14Feb2005 
      is = 0
	  ips = 0 ! Marco
      ...
      do
		 ...
		 elseif (keyword(1:7).eq.'MONITOL') then ! Marco - MONITOR LINE - 14Feb2005 
            iml = iml +1 ! Marco - MONITOR LINE - 14Feb2005 
            read(input_line(ileft:iright),*) x1_monitorl(iml),y1_monitorl(iml),& ! Marco - MONITOR LINE - 14Feb2005 
			x2_monitorl(iml),y2_monitorl(iml) ! Marco - MONITOR LINE - 14Feb2005	 
         elseif (keyword(1:8).eq.'SNAPSHOT') then
            is = is +1
	     ...
	     elseif (keyword(1:10).eq.'CHTMONITOR') then				! Marco 25Sep05
            read(input_line(ileft:iright),*) ndt_change_monitor ! Marco 25Sep05
	     elseif (keyword(1:10).eq.'CHTIMESTEP') then				! Marco 25Sep05
            read(input_line(ileft:iright),*) time_change_sampling ! Marco 25Sep05
		 elseif (keyword(1:8).eq.'SCALMESH') then				! Marco - SCALE MESH 11 May 2005
            read(input_line(ileft:iright),*) factor_scale_mesh	! Marco - SCALE MESH 11 May 2005						
         endif
      enddo
...
! Riordino gli snapshots
	  ...
	  if (time_change_sampling.eq.0) then	! Marco 25Sep05
		time_change_sampling = stop_time	! Marco 25Sep05
	  endif									! Marco 25Sep05
...
!     *****************************************************************
      ...
      subroutine scale_header(time_step,ndt_monitor,&
	                         nmonitors,x_monitor,y_monitor,&
							 nmonitorsl,&
							 x1_monitorl,y1_monitorl,&
							 x2_monitorl,y2_monitorl,&
							 factor_scale_mesh,&
							 ndt_change_monitor) !Marco 25Sep05				
      
!     © POLIMI, 2005, All Rights Reserved
!     Authors: Marco Stupazzini (5 May 2005)
!
      ...
      integer*4 :: nmonitors
	  integer*4 :: nmonitorsl ! Marco - MONITOR LINE - 14Feb2005 
      real*8, dimension(*) :: x_monitor,y_monitor
	  real*8, dimension(*) :: x1_monitorl,y1_monitorl ! Marco - MONITOR LINE - 14Feb2005
	  real*8, dimension(*) :: x2_monitorl,y2_monitorl ! Marco - MONITOR LINE - 14Feb2005 
	  ...
	  real*8 :: ndt_monitor
	  real*8 :: ndt_change_monitor	!Marco 25Sep05
	  real*8 :: time_step
      ...
      do i = 1,nmonitors
		...
		x_monitor(i) = x_monitor(i) * factor_scale_mesh
		y_monitor(i) = y_monitor(i) * factor_scale_mesh

	  enddo

	  do i = 1,nmonitorsl

         x1_monitorl(i) = x1_monitorl(i) * factor_scale_mesh
		 y1_monitorl(i) = y1_monitorl(i) * factor_scale_mesh
		 x2_monitorl(i) = x2_monitorl(i) * factor_scale_mesh
		 y2_monitorl(i) = y2_monitorl(i) * factor_scale_mesh

      enddo

	  ndt_monitor = ndt_monitor / factor_scale_mesh
	  ndt_change_monitor = ndt_change_monitor / factor_scale_mesh ! Marco 25Sep05
	  time_step = time_step	* factor_scale_mesh
      

      
      return
      end subroutine scale_header
      ...
      do
         ...
         elseif (keyword.eq.'FUNC') then
            ...
            elseif (func_type.eq.25) then
               nfunc_data = nfunc_data + 3
			elseif (func_type.eq.26) then
               nfunc_data = nfunc_data + 4
            elseif (func_type.eq.27) then
               nfunc_data = nfunc_data + 8
            elseif (func_type.eq.28) then
               nfunc_data = nfunc_data + 12
			elseif (func_type.eq.29) then
               nfunc_data = nfunc_data + 4
            elseif (func_type.eq.30) then
               nfunc_data = nfunc_data + 4
		    endif
         endif
      enddo
...
!
		 elseif (keyword.eq.'MATG') then
            ...
            read(input_line(ileft:iright),*) tagmg(img),typemg(img),&
				...
				elseif (typemg(img).eq.3) then !EGU2005
					...
					!                -----------
					!
					!	 Zone 1 = rho1, gamma1, lambda1, mu1
					!    Zone 2 = rho2, gamma2, lambda2, mu2
					!    The two areas are linked with a linear gradient
					!
					...
					!valmg(img,3) = mu1
					!valmg(img,4) = gamma1
					!valmg(img,5) = x_centro
					!valmg(img,6) = y_centro
					!valmg(img,7) = rho2
					!valmg(img,8) = lambda2
					!valmg(img,9) = mu2
					!valmg(img,10) = gamma2
					...
					alpha_Vs = (Vs2 - Vs1)/(valmg(img,12) - valmg(img,11))	!EGU2005
					beta_Vs = Vs1 - valmg(img,11) * alpha_Vs				!EGU2005
	

					valmg(img,13) = valmg(img,1) * alpha_Vs **2				!EGU2005
					valmg(img,14) = 2 * valmg(img,1) * alpha_Vs * beta_Vs	!EGU2005
					valmg(img,15) = valmg(img,1) * beta_Vs **2				!EGU2005

					!***********************************************************

					alpha_Vp = (Vp2 - Vp1)/(valmg(img,12) - valmg(img,11))	!EGU2005
					beta_Vp = Vp1 - valmg(img,11) * alpha_Vp				!EGU2005

					valmg(img,16) = valmg(img,1) * alpha_Vp **2				!EGU2005
					valmg(img,17) = 2 * valmg(img,1) * alpha_Vp * beta_Vp	!EGU2005
					valmg(img,18) = valmg(img,1) * beta_Vp **2				!EGU2005
		
					!***********************************************************
				
				! ESG2006													! ESG2006
				!															! ESG2006
				elseif (typemg(img).eq.4) then								! ESG2006
...
!
        ...
        elseif (keyword.eq.'VPCL') then ! Clara 
		 ...
		 valvpcl(ivpcl,3),valvpcl(ivpcl,4),valvpcl(ivpcl,5),&
		 valvpcl(ivpcl,6),& ! Clara  !Clara_non_ass
		 valvpcl(ivpcl,7),&   ! Clara  !M&C NON LOCAL 02Mar05
		 valvpcl(ivpcl,8),valvpcl(ivpcl,9) !Cam_Clay
!
...
!
         elseif (keyword.eq.'FUNC') then
            ...
            elseif (func_type(ifunc).eq.25) then
               ...
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
			elseif (func_type(ifunc).eq.26) then
               func_indx(ifunc +1) = func_indx(ifunc) + 4
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
            elseif (func_type(ifunc).eq.27) then
               func_indx(ifunc +1) = func_indx(ifunc) + 8
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
		    elseif (func_type(ifunc).eq.28) then
               func_indx(ifunc +1) = func_indx(ifunc) + 12
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
			elseif (func_type(ifunc).eq.29) then
               func_indx(ifunc +1) = func_indx(ifunc) + 4
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
            elseif (func_type(ifunc).eq.30) then
               func_indx(ifunc +1) = func_indx(ifunc) + 4
               read(input_line(ileft:iright),*)trash,trash,&
                    (func_data(j), j = func_indx(ifunc),func_indx(ifunc +1) -1)
            endif
...
!     *****************************************************************
      ...
      subroutine scale_material_el(ndX,valdX, &
                                  ndY,valdY, &
								  ndpX,valdpX, &
                                  ndpY,valdpY, &
								  nvpcl,valvpcl, &
...
!     © POLIMI, 2005, All Rights Reserved
!     Authors: Marco Stupazzini (11 May 2005)
!     
      implicit none
      
	  integer*4 :: ndX,ndY,ndpX,ndpY
      ...
      do i = 1,ndX
         ...
         valdX(i,1) = valdX(i,1) * factor_scale_mesh
		 valdX(i,2) = valdX(i,2) * factor_scale_mesh
	  
	  enddo

	  do i = 1,ndY
		
         valdY(i,1) = valdY(i,1) * factor_scale_mesh
		 valdY(i,2) = valdY(i,2) * factor_scale_mesh
	  
	  enddo

	  do i = 1,ndpX
		
         valdpX(i,1) = valdpX(i,1) * factor_scale_mesh
		 valdpX(i,2) = valdpX(i,2) * factor_scale_mesh
		 valdpX(i,3) = valdpX(i,3) * factor_scale_mesh
	  
	  enddo

	  do i = 1,ndpY
		
         valdpY(i,1) = valdpY(i,1) * factor_scale_mesh
		 valdpY(i,2) = valdpY(i,2) * factor_scale_mesh
		 valdpY(i,3) = valdpY(i,3) * factor_scale_mesh
	  
	  enddo

	  do i = 1,nvpcl 
		 valvpcl(i,7) = valvpcl(i,7) * factor_scale_mesh
	  enddo

	  do i = 1,nvpsa 
		 valvpsa(i,3) = valvpsa(i,3) * factor_scale_mesh
	  enddo

      
      return
      
      end subroutine scale_material_el
...
!     ********************************************************
      ...
      subroutine scale_grid_el(nnode,xx,yy,factor_scale_mesh)
...
!     
      ...
      do i = 1,nnode
        xx(i) = xx(i) * factor_scale_mesh
		yy(i) = yy(i) * factor_scale_mesh
      enddo
      
      return
      
      end subroutine scale_grid_el
...
!     
	  ...
	  real*8, dimension(nnode) :: vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read   ! Clara 23/04/05
               ...
               do iq = 1,nn
                  do ip = 1,nn
                     ...
                     sxx(in) = sxx(in) + (lambda + 2.0d0*mu) * (duxdx(ip,iq) - vistn1_0_read(in)) &
						  ...
						  + lambda * (- vistn4_0_read(in))    ! Clara 23/04/05
                     ...
                     szz(in) = szz(in) + lambda &
                          * (duxdx(ip,iq) - vistn1_0_read(in) + duydy(ip,iq) - vistn2_0_read(in))&
						  + (lambda + 2.0d0*mu) * (- vistn4_0_read(in))    ! Clara 23/04/05
                     sxy(in) = sxy(in) + mu * (duxdy(ip,iq) + duydx(ip,iq) -  vistn3_0_read(in)) !Clara 13/02/07
...
!     
      ...
      real*8, dimension(nn,nn) :: sxx,syy,sxy,szz,fx,fy
	  ...
	  real*8 :: devia1,devia2,devia3,devia4 !Clara 17/04/05
      real*8 :: avect1,avect2,avect3,avect4 !Clara 17/04/05
      real*8 :: vivel1,vivel2,vivel3,vivel4 ! Clara 23/04/05 
	  real*8 :: vistn1,vistn2,vistn3,vistn4 ! Clara 23/04/05  
	  ...
	  real*8, dimension(nn,nn,nelem) :: yield_el ! Marco 4Apr2005 - Output yield
	  real*8, dimension(nn,nn,nelem) :: vnucl_el ! Marco 4Apr2005 - Output yield
	  ...
	  real*8, dimension(nn,nn,nelem) :: vistn4_0 ! Clara 23/04/05
	  real*8  :: smean,e,temp  !Cam_Clay
	  real*8, dimension(nn,nn,nelem) :: pcc_el !Cam_Clay
	  ...
	  real*8, dimension(15) :: u            !MCNAP vpsa
      real*8, dimension(3,3):: a,ak,D,DV,DPP,ss   !MCNAP vpsa
      real*8 :: al,csi,z14,bef,pc,dens      !MCNAP vpsa
	  integer*4 :: j,k                      !MCNAP vpsa
	
	  real*8 :: destn1,destn2,destn3,destn4,debar,dt,p1 !Clara 
	  ...
	  real*8, dimension(nvpcl,nnt) :: yield1_az_vpcl ! M NON LOCAL 02Mar05
	  real*8, dimension(nvpcl,nnt) :: yield2_az_vpcl ! M NON LOCAL 02Mar05
	  !real*8, dimension(nnt) :: yield_new ! M NON LOCAL 02Mar05
	  ...
	  real*8, dimension(nvpcl,sum_end_nloc_vpcl) :: weigth_nloc_vpcl
	  integer*4 :: i_nloc
	  integer*4, dimension(nnt) :: nodal_counter
	  integer*4, dimension(nnt) :: check_in_vpcl
	  real*8 :: det, PI
	  real*8, dimension(nvpcl,nnt) :: integral_nloc_vpcl	!dead line
	  ...
	  real*8, dimension(nvpsa,nnt) :: yield1_az_vpsa ! M NON LOCAL 02Mar05
	  real*8, dimension(nvpsa,nnt) :: yield2_az_vpsa ! M NON LOCAL 02Mar05
	  ...
	  real*8, dimension(nvpsa,sum_end_nloc_vpsa) :: weigth_nloc_vpsa
	  integer*4, dimension(nnt) :: check_in_vpsa
	  real*8, dimension(nvpsa,nnt) :: integral_nloc_vpsa


	  real*8 :: beta,mmm,aaa1   !Clara-new definition nucleus viscous



	  real*8 :: radius_s ! EGU2005
	  real*8 :: D_s ! ESG2006
      ...
      do iq = 1,nn   
         do ip = 1,nn
		    ...
		    if (which_matg.ne.0) then ! Mexico Paco grad_lin 29.11.2004
				do imatg = 1,nmatg ! Mexico Paco grad_lin 29.11.2004
					if (tagmat.eq.tagmatg(imatg)) then ! Mexico Paco grad_lin 29.11.2004
					  ...
					  ! CIRCULAR GRADIENT									! EGU2005
					  !														! EGU2005
					  elseif (typematg(imatg).eq.3) then					! EGU2005
																			! EGU2005
					    radius_s = ((xs(in) - valmatg(imatg,5))**2 + &		! EGU2005
						            (ys(in)-  valmatg(imatg,6))**2 )**.5	! EGU2005
																			! EGU2005
						mu = valmatg(imatg,13) * radius_s**2 + &			! EGU2005
							 valmatg(imatg,14) * radius_s + &				! EGU2005
					  		 valmatg(imatg,15)								! EGU2005
																			! EGU2005
						lambda = valmatg(imatg,16) * radius_s**2 + &		! EGU2005
								 valmatg(imatg,17) * radius_s + &			! EGU2005
				      			 valmatg(imatg,18) - 2*mu					! EGU2005


					  ! ESG2006 - Grenoble									! ESG2006
					  !														! ESG2006
					  ! VS  = 300.0 + 19.0 * sqrt(D)						! ESG2006                             
					  ! VP  = 1450.0 + 1.2 * D								! ESG2006                               
					  ! RHO = 2140.0 + 0.125 * D							! ESG2006                            
					  !														! ESG2006                                             
					  ! where												! ESG2006                                                     
					  !														! ESG2006                                            
					  ! D: depth in m										! ESG2006                                             
					  ! VS: S velocity in m/s								! ESG2006                                     
					  ! VP: P velocity in m/s								! ESG2006                           
					  ! RHO: MASS DENSITY in kg/m^3							! ESG2006                       
																			! ESG2006
																			! ESG2006
					  elseif (typematg(imatg).eq.4) then					! ESG2006
																			! ESG2006
					    D_s = ( valmatg(imatg,1) - ys(in) )					! ESG2006
																			! ESG2006
						mu = 192600000 + &									! ESG2006
							 783790 * D_s + &								! ESG2006
							 45.125 * D_s**2 + &							! ESG2006
							 12198000 * (D_s**.5) + &						! ESG2006
							 712.5 * (D_s**.5) * D_s						! ESG2006
																			! ESG2006
						lambda = 4499350000. + &							! ESG2006
								 3986412.5 * D_s + &						! ESG2006
				      			 3299.1 * D_s**2 + &						! ESG2006
								 0.18 * D_s**3								! ESG2006
																			! ESG2006
					  endif
            ...
            sxx(ip,iq) = (lambda +2.0d0*mu) * (duxdx(ip,iq) - vistn1_0(ip,iq,ielem)) & ! Clara 
                         ...
                         + lambda * (- vistn4_0(ip,iq,ielem))               ! Clara 23/04/05       
			syy(ip,iq) = (lambda +2.0d0*mu) * (duydy(ip,iq) - vistn2_0(ip,iq,ielem)) & ! Clara 
                         + lambda * (duxdx(ip,iq) - vistn1_0(ip,iq,ielem))& ! Clara
						 + lambda * (- vistn4_0(ip,iq,ielem))               ! Clara 23/04/05 
            sxy(ip,iq) = mu * (duxdy(ip,iq) + duydx(ip,iq) - vistn3_0(ip,iq,ielem)) ! Clara 13/02/07
			
			szz(ip,iq) = lambda * (duxdx(ip,iq) - vistn1_0(ip,iq,ielem)) & ! Clara 
                         + lambda * (duydy(ip,iq) - vistn2_0(ip,iq,ielem))& ! Clara 
                         +(lambda +2.0d0*mu) * (- vistn4_0(ip,iq,ielem))     ! Clara 23/04/05 
            ...
            !sxy(ip,iq) = sxy(ip,iq) - (mu * (vistn3_0(ip,iq,ielem) + vistn3_0(ip,iq,ielem)))
			 ...
			 is = nn*(iq -1) +ip            !Cam_Clay
             in = cs(cs(ielem -1) + is)     !Cam_Clay
                
		     temp = rho0*9.81*abs(1.d0-ys(in))                           !Cam_Clay
			 sxx(ip,iq) = sxx(ip,iq) - temp/2.                        !Cam_Clay
			 syy(ip,iq) = syy(ip,iq) - temp                           !Cam_Clay
			 szz(ip,iq) = szz(ip,iq) - temp/2.
...
!   STRESS CALCULATION - MATERIAL VISCO-PLASTIC
	 ...
	 ! ****************************
		...
		do ivpcl = 1,nvpcl
		    ...
		    !loading_slopeCamClay  =val_vpcl_el(im,8) !lambda Cam_Clay
			!unlaoding_slopeCamCl  =val_vpcl_el(im,9) !kappa Cam_Clay 
				...
				do iq = 1,nn   
					...
					call invarg (devia1,devia2,devia3,devia4, &
								valvpcl(which_vpcl,1),valvpcl(which_vpcl,2),valvpcl(which_vpcl,3), &
								valvpcl(which_vpcl,4),valvpcl(which_vpcl,5), &
								steff, smean, &   !Cam_Clay
								sxx(ip,iq),syy(ip,iq),sxy(ip,iq),szz(ip,iq), &
								theta, varj2, yield_el(ip,iq,ielem),& ! Marco 4Apr2005 - Output yield
								fvpcl(which_vpcl))

				    

					if (valvpcl(ivpcl,7).gt.0) then							! M NON LOCAL 02Mar05
						is = nn*(iq -1) +ip									! M NON LOCAL 02Mar05
						in = cs(cs(ielem -1) + is)

						!write(54,*) yield,in,iq,ip,ielem

													! M NON LOCAL 02Mar05						
						do i_nloc = begin_nloc_vpcl(ivpcl,in),end_nloc_vpcl(ivpcl,in)		! M NON LOCAL 02Mar05
							!if (i_nloc.ne.in) then
								det_j = dxdx(iq)*dydy(ip) - dxdy(ip)*dydx(iq)
								det_j = dabs(det_j)
								yield1_az_vpcl(ivpcl,connectivity_nloc_vpcl(ivpcl,i_nloc)) = yield1_az_vpcl(ivpcl,connectivity_nloc_vpcl(ivpcl,i_nloc)) + yield_el(ip,iq,ielem) * &
									weigth_nloc_vpcl(ivpcl,i_nloc) * &
									ww(iq) * ww(ip) * det_j !/ nodal_counter(connectivity_nloc(ivpcl,i_nloc))	    
							!endif
							!yield1_az(ivpcl,connectivity_nloc(ivpcl,i_nloc)) = yield1_az(ivpcl,connectivity_nloc(ivpcl,i_nloc)) + yield
							
						enddo

						!yield_new(in) = yield_new(in) + yield
								
						if (its.ge.2) yield_el(ip,iq,ielem) = yield2_az_vpcl(ivpcl,in)   ! M NON LOCAL 02Mar05
						!if (its.ge.2) write(54,*) yield,in,iq,ip,ielem

					endif											! M NON LOCAL 02Mar05

					call yieldf (avect1,avect2,avect3,avect4,&
								devia1,devia2,devia3,devia4,&
								valvpcl(which_vpcl,1),valvpcl(which_vpcl,2),valvpcl(which_vpcl,3), &
								valvpcl(which_vpcl,4),valvpcl(which_vpcl,6), &  !!Clara_non_ass
								steff, smean, &  !Cam_Clay
								sxx(ip,iq),syy(ip,iq),sxy(ip,iq),szz(ip,iq), &   !Cam_Clay 
								theta,varj2,fvpcl(which_vpcl))

					call flowvp (valvpcl(which_vpcl,1),valvpcl(which_vpcl,2),valvpcl(which_vpcl,3), &
								valvpcl(which_vpcl,4),valvpcl(which_vpcl,5), &
								devia1,devia2,devia3,devia4,&
								avect1,avect2,avect3,avect4,&
								vivel1,vivel2,vivel3,vivel4,&
								vivel_el(ip,iq,ielem),pcc_el(ip,iq,ielem),&    !Cam_Clay
								yield_el(ip,iq,ielem),& ! Marco 4Apr2005 - Output yield
								vnucl_el(ip,iq,ielem),& ! Marco 4Apr2005 - Output yield
								vivel_length,fvpcl(which_vpcl))
					...
					destn2 = vivel2 * dt
					destn3 = vivel3 * dt
					destn4 = vivel4 * dt   ! Clara 23/04/05 
					!destn1 = 0.0
					!destn2 = 0.0
					!destn3 = 0.0
					!destn4 = 0.0
					...
					if (fvpcl(which_vpcl).eq.5)then     !Cam_Clay
					    e=(2.65*9810.-rho0*9.81)/(rho0*9.81-9810.)    !Cam_Clay
					    pcc_el(ip,iq,ielem) = pcc_el(ip,iq,ielem)-pcc_el(ip,iq,ielem)*(1.d0+e)*(destn1+destn2+destn4)/ &   !Cam_Clay
					                 (valvpcl(which_vpcl,8)-valvpcl(which_vpcl,9))                    !Cam_Clay
				!		write(*,*)e,destn2,pcc_el(ip,iq,ielem)
					endif                               !Cam_Clay
					...
					debar=dsqrt((2.0*(destn1**2 + destn2**2 + destn3**2 + destn4**2)/3.0))  ! Clara 23/04/05 
			
				!!BISOGNA CAMBIARE LA DEFINIZIONE SECONDO L'ARTICOLO RIG!!RICORDARSI
					...
					vistn4_0(ip,iq,ielem) = vistn4_0(ip,iq,ielem) + destn4    ! Clara 23/04/05 
...
!
                        ...
                        call PAR(dens,u,& 
		                         ...
		                         valvpsl(which_vpsl,13),valvpsl(which_vpsl,14),&         !Clara-new definition nucleus viscous
		                         valvpsd_l(which_vpsd,1),valvpsd_l(which_vpsd,2),&		!Marco 18May2005 - Speed-up changes 6% improvement
	                          	 valvpsd_l(which_vpsd,3),valvpsd_l(which_vpsd,4),&		!Marco 18May2005 - Speed-up changes 6% improvement
		                         valvpsd_l(which_vpsd,5),valvpsd_l(which_vpsd,6),&		!Marco 18May2005 - Speed-up changes 6% improvement
		                         valvpsd_l(which_vpsd,7),valvpsd_l(which_vpsd,8),&		!Marco 18May2005 - Speed-up changes 6% improvement
	                           	 valvpsd_l(which_vpsd,9),valvpsd_l(which_vpsd,10),&		!Marco 18May2005 - Speed-up changes 6% improvement
		                         valvpsd_l(which_vpsd,11),valvpsd_l(which_vpsd,12),&	!Marco 18May2005 - Speed-up changes 6% improvement
		                         valvpsd_l(which_vpsd,13),valvpsd_l(which_vpsd,14))		!Clara-new definition nucleus viscous
                     ...
                     !      MATRICES CLEARING
                       ...
                       call leg(s,ak,ss,p1,D,DV,dt,csi,bef,pc,u,yield_el(ip,iq,ielem))
                       
					   !Clara 050505_NON LOCAL
					   ...
					   if (valvpsa(ivpsa,3).gt.0) then							! M NON LOCAL 02Mar05
						...
						!write(54,*) yield,in,iq,ip,ielem
													...
													! M NON LOCAL 02Mar05						
						do i_nloc = begin_nloc_vpsa(ivpsa,in),end_nloc_vpsa(ivpsa,in)		! M NON LOCAL 02Mar05
							!if (i_nloc.ne.in) then
								det_j = dxdx(iq)*dydy(ip) - dxdy(ip)*dydx(iq)
								det_j = dabs(det_j)
								yield1_az_vpsa(ivpsa,connectivity_nloc_vpsa(ivpsa,i_nloc)) = yield1_az_vpsa(ivpsa,connectivity_nloc_vpsa(ivpsa,i_nloc)) + yield_el(ip,iq,ielem) * &
									weigth_nloc_vpsa(ivpsa,i_nloc) * &
									ww(iq) * ww(ip) * det_j !/ nodal_counter(connectivity_nloc(ivpsa,i_nloc))	    
							!endif
							!yield1_az(ivpsa,connectivity_nloc(ivpsa,i_nloc)) = yield1_az(ivpsa,connectivity_nloc(ivpsa,i_nloc)) + yield
							
						enddo

						!yield_new(in) = yield_new(in) + yield
								
						if (its.ge.2) yield_el(ip,iq,ielem) = yield2_az_vpsa(ivpsa,in)   ! M NON LOCAL 02Mar05
						!if (its.ge.2) write(54,*) yield,in,iq,ip,ielem
                    ...
                    if (yield_el(ip,iq,ielem).le.valvpsa(ivpsa,4)) then							                    !Clara-new definition nucleus viscous
                      !	    write(*,*)ff
                      	  vnucl_el(ip,iq,ielem)=u(13)*p1*exp(u(14)*yield_el(ip,iq,ielem))		                    !Clara-new definition nucleus viscous
                    else
                      !	    write(*,*)ff
                         
						  
						  aaa1=u(13)*u(14)*exp(u(14)*valvpsa(ivpsa,4))*valvpsa(ivpsa,4)*u(15)/(1/log(10.))**(1/u(15))

						  beta=(u(13)*exp(u(14)*valvpsa(ivpsa,4))/(aaa1**(1/(1-u(15)))*(1/log(10.))**(1/u(15))))**((1-u(15))/-u(15))
                          mmm=(exp((aaa1/beta)**(u(15)/(1-u(15))) ))/valvpsa(ivpsa,4)      
                         
                          vnucl_el(ip,iq,ielem)=beta*((log10(mmm*yield_el(ip,iq,ielem)))**(1/u(15)))*p1   !Clara-new definition nucleus viscous
                    endif

                    !vnucl_el(ip,iq,ielem)=u(13)*p1*exp(u(14)*yield_el(ip,iq,ielem))

      
	                do i = 1,3
	                 do j = 1,3
	   	               DPP(i,j)=SS(i,j)*vnucl_el(ip,iq,ielem)*DT
                     enddo
					 DEP(i)=DPP(i,i)
                    enddo
   ...
   !   write(*,*)dep
					   ...
					   call DENSITA(dens,dep,valvpsl(which_vpsl,15),valvpsd(which_vpsd,15))
	...
	!!BISOGNA CAMBIARE LA DEFINIZIONE SECONDO L'ARTICOLO RIG!!RICORDARSI
					...
					!write(*,*)ip,iq,destn2,vistn2_0(1,1,1),epn2(1,1,1),densit(1,1,1)
...
!     ********************************************************
      ...
      subroutine make_absorbing_force(lambda0,mu0,rho0,&
									  nmatg,valmatg,which_matg,tagmatg,typematg,& ! Mexico Paco grad_lin 29.11.2004
									  tagmat,&
									  nn,ct,ww,dd,&
                                      dxdx,dxdy,dydx,dydy,&
...
!     
      ...
      real*8 :: det_j,nnx2,nny2,term_k
	  real*8, dimension(nn,nn) :: term,term11,term12,term21,term22
	  
	  integer*4 :: nnt
	  real*8, dimension(nnt) :: xs,ys ! Clara 
	  integer*4 :: cs_nnz,is,in,ielem
	  integer*4, dimension(0:cs_nnz) :: cs
	  real*8 :: radius_s
		    ...
		    if (which_matg.ne.0) then ! Mexico Paco grad_lin 29.11.2004
				do imatg = 1,nmatg ! Mexico Paco grad_lin 29.11.2004
					if (tagmat.eq.tagmatg(imatg)) then ! Mexico Paco grad_lin 29.11.2004
					  ...
					  is = nn*(j -1) +i ! Mexico Paco grad_lin 29.11.2004
					  in = cs(cs(ielem -1) + is) ! Mexico Paco grad_lin 29.11.2004
					  ...
					  ! CIRCULAR GRADIENT								! EGU2005
					  elseif (typematg(imatg).eq.1) then					! EGU2005
					   
					    radius_s = ((xs(in) - valmatg(imatg,5))**2 + &
						            (ys(in)-  valmatg(imatg,6))**2 )**.5			! EGU2005
	...
	!*************************************************************
		...
		term(i,j) = 0.5d0*ll * (2.0d0*mu - dsqrt(mu * (lambda+2.0d0*mu)))
      
		term11(i,j) = nnx2 * dsqrt(lambda +2.0d0*mu) + nny2 * dsqrt(mu)
		term11(i,j) = -0.5d0 * ll * dsqrt(rho) * term11(i,j)
      
		term12(i,j) = (dsqrt(mu) - dsqrt(lambda +2.0d0*mu)) * nnx * nny
		term12(i,j) = 0.5d0 * ll * dsqrt(rho) * term12(i,j)
      
		term21(i,j) = nny2 * dsqrt(lambda +2.0d0*mu) + nnx2 * dsqrt(mu)
		term21(i,j) = -0.5d0 * ll * dsqrt(rho) * term21(i,j)
      
		term22(i,j) = (dsqrt(mu) - dsqrt(lambda +2.0d0*mu)) * nnx * nny
		term22(i,j) = 0.5d0 * ll * dsqrt(rho) * term22(i,j)
      ...
      do il = ia,ib
         do im = ja,jb
            ...
            term_k = term(il,im) * (dydy(il)*nny + dxdy(il)*nnx) &
                   * ww(j) / det_j
            ...
            term_k = -1.0d0 * term(il,im) * (dydx(im)*nny + dxdx(im)*nnx) &
                    * ww(j) / det_j
            ...
            fx(il,im) = fx(il,im) -term11(il,im)*ww(j) *vx(il,im)
            fx(il,im) = fx(il,im) -term12(il,im)*ww(j) *vy(il,im)
            
            fy(il,im) = fy(il,im) -term21(il,im)*ww(j) *vy(il,im)
            fy(il,im) = fy(il,im) -term22(il,im)*ww(j) *vx(il,im)
...
!     ********************************************************
      ...
      subroutine invarg(devia1,devia2,devia3,devia4, &
			            fdatm,hards,gamma, &
                        ddelta,frict, &
			            steff, smean, & !Cam_Clay
                        stemp1,stemp2,stemp3,stemp4, &
			            theta, varj2, yield, ncrit)
...
! 
      ...
      real*8 :: devia1,devia2,devia3,devia4
      real*8 :: fdatm,hards,gamma,ddelta,frict
      real*8 :: steff
	  real*8 :: stemp1,stemp2,stemp3,stemp4
      real*8 :: theta, varj2, yield
      integer*4 :: ncrit

	  real*8 :: root3,smean,varj3,sint3,phira,snphi
	  real*8 :: m,q    !Cam_Clay
...
!      Invariants for plane stress and strain conditions
       ...
       !smean  = (stemp1 + stemp2 + stemp3 )/3.0    !Clara 17/04/05
	   smean  = (stemp1 + stemp2 + stemp4 )/3.0    !Marco 17/04/05
       devia1 = stemp1 - smean
       devia2 = stemp2 - smean
       devia3 = stemp3
	   devia4 = stemp4 -smean !Clara 17/04/05
 
       !varj2 = devia3**2 + 0.5 * (devia1**2 + &
       !       devia2**2)
       !varj3 = 0.0d0
        
       !varj2 = devia3**2 + 0.5 * (devia1**2 + devia2**2 + devia3**2)  !Clara 17/04/05
	   varj2 = devia3**2 + 0.5 * (devia1**2 + devia2**2 + devia4**2)  !Marco 17/04/05
	   varj3 = devia1*devia2*devia4-devia4*(devia3**2)                !Clara 17/04/05
	  ...
	  !Cam_Clay                         !Cam_Clay
	  elseif (ncrit.eq.5) then          !Cam_Clay
	        phira=frict*0.017453292     !Cam_Clay
			snphi=sin(phira)            !Cam_Clay
			m=6.0*snphi/(3.0-snphi)     !Cam_Clay
			!q=dsqrt(varj2*3.0/2.)       !Cam_Clay    controllare che ci voglia 3/2
			q=root3*steff
			yield=-smean-((q/m)**2.)/smean  !Cam_Clay  
			!write(*,*)m,q,smean,yield
...
!     ********************************************************
      ...
      subroutine yieldf(avect1,avect2,avect3,avect4, &
			            devia1,devia2,devia3,devia4, &
			            fdatm,hards,gamma, &
                        ddelta,frictpot, &  !!Clara_non_ass
						steff,smean,&  !Cam_Clay
						stemp1,stemp2,stemp3,stemp4,&  !Cam_Clay
						theta,varj2,ncrit)
...
!     Obtain: plastic vectors df/ds
      ...
      real*8 :: theta, varj2
      integer*4 :: ncrit
	  real*8 :: m,q,smean    !Cam_Clay
	  real*8 :: stemp1,stemp2,stemp3,stemp4    !Cam_Clay

	  real*8 :: veca11,veca12,veca13,veca14  !Clara 17/04/05
	  real*8 :: veca21,veca22,veca23,veca24  !Clara 17/04/05
	  real*8 :: veca31,veca32,veca33,veca34  !Clara 17/04/05
      ...
      veca23=devia3/steff
	  veca24=devia4/(2.0*steff) !Clara 17/04/05

      ! Get vector a3

      veca31=devia2*devia4+varj2/3.0          !Clara 17/04/05
      veca32=devia1*devia4+varj2/3.0          !Clara 17/04/05
      veca33=-2.0*devia3*devia4               !Clara 17/04/05
      veca34=devia1*devia2-devia3*2+varj2/3.0 !Clara 17/04/05
	 ...
	 elseif  (ncrit.eq.5) then    !Cam_Clay
	        ...
	        snphi=sin(frictpot*0.017453292)    !Cam_Clay  	
			m=6.0*snphi/(3.0-snphi)     !Cam_Clay
			!q=dsqrt(varj2*3.0/2.)       !Cam_Clay    controllare che ci voglia 3/2
			q=root3*steff
			  
			cons1=1.0-(-q/smean/m)**2.   !Cam_Clay
			cons2=2.*(-q/smean/(m**2.))  !Cam_Clay
			cons3=0.0                    !Cam_Clay

			veca11=-1.0/3.               !Cam_Clay
            veca12=-1.0/3.               !Cam_Clay
            veca13=0.0                   !Cam_Clay
	        veca14=-1.0/3.               !Cam_Clay

			veca21=(2.*stemp1-stemp2-stemp4)/(2.0*q)    !Cam_Clay
            veca22=(2.*stemp2-stemp1-stemp4)/(2.0*q)    !Cam_Clay
            veca23=6.*stemp3/(2.0*q)                    !Cam_Clay
	        veca24=(2.*stemp4-stemp1-stemp2)/(2.0*q)    !Cam_Clay 
	  ...
	  avect4=cons1 * veca14 + cons2 * veca24 + cons3 * veca34  !Clara 17/04/05
...
!     ********************************************************
      ...
      subroutine flowvp(fdatm,hards,gamma, &
			            ...
			            avect1,avect2,avect3,avect4, &
						vivel1,vivel2,vivel3,vivel4, &
						vivel,pcc,yield,fnorm,npoin,ncrit)  !Cam_Clay
...
!
	  ...
	  real*8 :: devia1,devia2,devia3,devia4    ! Clara 23/04/05 
	  real*8 :: avect1,avect2,avect3,avect4    ! Clara 23/04/05 
      real*8 :: fdatm,hards,gamma,ddelta,frict
	  integer*4 :: npoin
	  real*8 :: vivel1,vivel2,vivel3,vivel4    ! Clara 23/04/05 
      real*8 :: vivel
	...
	!	frict=frict*0.017453292
		...
		if(ncrit.eq.1.or.ncrit.eq.2.or.ncrit.eq.3.or.ncrit.eq.4) fdatm1=fdatm1 + vivel * hards  !Cam_Clay                             
	    if(ncrit.eq.5)fdatm1=pcc   !Cam_Clay
      ...
      if (fnorm.lt.0.0) then
         ...
         vivel2 = 0.0
		 vivel3 = 0.0
		 vivel4 = 0.0  ! Clara 23/04/05 
		 !if (fnorm.lt.-1) fnorm = -1.0d0 ! VISUALIZATION ONLY!!!
	  else
	      cmult= gamma*(fnorm**ddelta)
		  avect1 = avect1 * cmult
		  avect2 = avect2 * cmult
		  avect3 = avect3 * cmult
		  avect4 = avect4 * cmult ! Clara 23/04/05 

          vivel1 = avect1
		  vivel2 = avect2
		  vivel3 = avect3
		  vivel4 = avect4    ! Clara 23/04/05 

      endif

!	  write(*,*)fdatm1,fnorm,cmult,avect1,avect2,avect3,avect4
...
!     Controllare output variabili geotecniche:p,q,dev,dep --->>CLARA!
      ...
      real*8, dimension(:), allocatable :: ux,uy,vivel_read,vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read ! Clara 23/04/05 
      real*8, dimension(:), allocatable :: yield_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: vnucl_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(:), allocatable :: sxx,syy,szz,sxy
      ...
      real*8 :: val
      integer*4 :: istep,nsteps      
      character*70 :: in_fileE,in_fileV,in_fileT1,in_fileT2,in_fileT3,out_file,in_fileT4 ! Clara 23/04/05
	  character*70 :: in_fileY ! Marco 4Apr2005 - Output yield
	  character*70 :: in_fileF ! Marco 4Apr2005 - Output yield
      character*40 :: elcode
      integer*4 :: lname
      ...
      lname = len_trim(file_name)
      in_fileE = file_name(1:lname) // 'E_xxx_xxx.bin' ! DISPLACEMENT
	  in_fileV = file_name(1:lname) // 'V_xxx_xxx.bin' ! VIVEL
	  in_fileY = file_name(1:lname) // 'Y_xxx_xxx.bin' ! Marco 4Apr2005 - Output yield
	  in_fileF = file_name(1:lname) // 'F_xxx_xxx.bin' ! Marco 4Apr2005 - Output yield
      ...
      in_fileT4 = file_name(1:lname) // '8_xxx_xxx.bin'  ! Clara 23/04/05
      out_file = file_name(1:lname) // '.inp'

      istep = 1
      allocate(ux(nnode),uy(nnode),vivel_read(nnode),vistn1_0_read(nnode),vistn2_0_read(nnode),vistn3_0_read(nnode),vistn4_0_read(nnode)) ! Clara 23/04/05 
      allocate(yield_read(nnode)) ! Marco 4Apr2005 - Output yield
	  allocate(vnucl_read(nnode)) ! Marco 4Apr2005 - Output yield
	  allocate(sxx(nnode),syy(nnode),szz(nnode),sxy(nnode))
      ...
      do count = 1,nsnaps
		...
		if (count.lt.10) then
			...
			write(in_fileY(lname+3:lname+4),'(a2)')'00'		! Marco 4Apr2005 - Output yield
            write(in_fileY(lname+5:lname+5),'(i1)')count	! Marco 4Apr2005 - Output yield

			write(in_fileF(lname+3:lname+4),'(a2)')'00'		! Marco 4Apr2005 - Output yield
            write(in_fileF(lname+5:lname+5),'(i1)')count	! Marco 4Apr2005 - Output yield
			...
			write(in_fileT4(lname+3:lname+4),'(a2)')'00'    ! Clara 23/04/05
            write(in_fileT4(lname+5:lname+5),'(i1)')count   ! Clara 23/04/05
			...
			write(in_fileY(lname+3:lname+3),'(a1)')'0'		! Marco 4Apr2005 - Output yield
			write(in_fileY(lname+4:lname+5),'(i2)')count	! Marco 4Apr2005 - Output yield

			write(in_fileF(lname+3:lname+3),'(a1)')'0'		! Marco 4Apr2005 - Output yield
			write(in_fileF(lname+4:lname+5),'(i2)')count	! Marco 4Apr2005 - Output yield
			...
			write(in_fileT4(lname+3:lname+3),'(a1)')'0'    ! Clara 23/04/05
            write(in_fileT4(lname+4:lname+5),'(i2)')count  ! Clara 23/04/05
			...
			write(in_fileY(lname+3:lname+5),'(i3)')count	! Marco 4Apr2005 - Output yield

			write(in_fileF(lname+3:lname+5),'(i3)')count	! Marco 4Apr2005 - Output yield
            ...
            write(in_fileT4(lname+3:lname+5),'(i3)')count     ! Clara 23/04/05
         ...
         do ip = 0,np -1
            if (ip.lt.10) then
			   ...
			   write(in_fileY(lname+7:lname+8),'(a2)')'00'	! Marco 4Apr2005 - Output yield
               write(in_fileY(lname+9:lname+9),'(i1)')ip	! Marco 4Apr2005 - Output yield

			   write(in_fileF(lname+7:lname+8),'(a2)')'00'	! Marco 4Apr2005 - Output yield
               write(in_fileF(lname+9:lname+9),'(i1)')ip	! Marco 4Apr2005 - Output yield
			   ...
			   write(in_fileT4(lname+7:lname+8),'(a2)')'00'     ! Clara 23/04/05
               write(in_fileT4(lname+9:lname+9),'(i1)')ip       ! Clara 23/04/05
			   ...
			   write(in_fileY(lname+7:lname+7),'(a1)')'0'	! Marco 4Apr2005 - Output yield
               write(in_fileY(lname+8:lname+9),'(i2)')ip	! Marco 4Apr2005 - Output yield

			   write(in_fileF(lname+7:lname+7),'(a1)')'0'	! Marco 4Apr2005 - Output yield
               write(in_fileF(lname+8:lname+9),'(i2)')ip	! Marco 4Apr2005 - Output yield
			   ...
			   write(in_fileT4(lname+7:lname+7),'(a1)')'0'     ! Clara 23/04/05
               write(in_fileT4(lname+8:lname+9),'(i2)')ip      ! Clara 23/04/05
               ...
               write(in_fileE(lname+7:lname+9),'(i3)')ip
				...
				write(in_fileY(lname+7:lname+9),'(i3)')ip ! Marco 4Apr2005 - Output yield

				write(in_fileF(lname+7:lname+9),'(i3)')ip ! Marco 4Apr2005 - Output yield

				write(in_fileT1(lname+7:lname+9),'(i3)')ip
				write(in_fileT2(lname+7:lname+9),'(i3)')ip
				write(in_fileT3(lname+7:lname+9),'(i3)')ip
				write(in_fileT4(lname+7:lname+9),'(i3)')ip     ! Clara 23/04/05
			...
			open(20,file=in_fileY)					! Marco 4Apr2005 - Output yield
            
            read(20,*)nd							! Marco 4Apr2005 - Output yield
            
            do i = 1,nnode							! Marco 4Apr2005 - Output yield
               read(20,*)in,val						! Marco 4Apr2005 - Output yield
               if (dabs(val).lt.1.d-30) val=0.d0	! Marco 4Apr2005 - Output yield
               yield_read(i) = val					! Marco 4Apr2005 - Output yield

            enddo									! Marco 4Apr2005 - Output yield
            
            close(20)								! Marco 4Apr2005 - Output yield


			open(20,file=in_fileF)					! Marco 4Apr2005 - Output yield
            ...
            do i = 1,nnode							! Marco 4Apr2005 - Output yield
               read(20,*)in,val						! Marco 4Apr2005 - Output yield
               if (dabs(val).lt.1.d-30) val=0.d0	! Marco 4Apr2005 - Output yield
               vnucl_read(i) = val					! Marco 4Apr2005 - Output yield
            ...
            open(20,file=in_fileT4)                     ! Clara 23/04/05
            
            read(20,*)nd                                ! Clara 23/04/05
            
            do i = 1,nnode                              ! Clara 23/04/05
               read(20,*)in,val                         ! Clara 23/04/05
               if (dabs(val).lt.1.d-30) val=0.d0        ! Clara 23/04/05
               vistn4_0_read(i) = val                   ! Clara 23/04/05

            enddo                                       ! Clara 23/04/05
            
            close(20)                                   ! Clara 23/04/05
         ...
         if ((opt.eq.2).or.(opt.eq.3).or.(opt.eq.7)) then
			...
			do i = 1,nnode
               write(1022,'(I8,2X,E14.6,2X,E14.6,2X,E14.6)')&
                    i,duxdx_average(i),duydy_average(i),(duydx_average(i)+duxdy_average(i)) !Clara output 28/11/06
            enddo
!Clara begin
            ...
            do i = 1,nnode
			   value= [-duxdx_average(i),-duydy_average(i),0.00d0,-(duydx_average(i)+duxdy_average(i))]   !definire le deformazioni !Clara output 28/11/06
...
!Clara end
		 ...
		 if (opt.eq.7) then					! Marco 4Apr2005 - Output yield
			
			write(1022,*) '   yield       1 ', tsnaps(count), '  1   1   0'	! Marco 4Apr2005 - Output yield

            if (istep.lt.10) then											! Marco 4Apr2005 - Output yield
               write(21,'(A4,I1,A3,E14.6)')'step',istep,' , ',tsnaps(count)	! Marco 4Apr2005 - Output yield
            else															! Marco 4Apr2005 - Output yield
               write(21,'(A4,I2,A3,E14.6)')'step',istep,' , ',tsnaps(count)	! Marco 4Apr2005 - Output yield
            endif															! Marco 4Apr2005 - Output yield
            
            write(21,*)'1 0 '				! Marco 4Apr2005 - Output yield
            write(21,*)'1 1 '				! Marco 4Apr2005 - Output yield
            write(21,*)'Yield, NO_UNITS'	! Marco 4Apr2005 - Output yield
            
            do i = 1,nnode					! Marco 4Apr2005 - Output yield
               write(21,'(I8,2X,E14.6)')&	! Marco 4Apr2005 - Output yield
                    i,yield_read(i)			! Marco 4Apr2005 - Output yield
			   write(1022,'(I8,2X,E14.6)')&	! Marco 4Apr2005 - Output yield
                    i,yield_read(i)			! Marco 4Apr2005 - Output yield
            enddo							! Marco 4Apr2005 - Output yield
				
            istep = istep + 1				! Marco 4Apr2005 - Output yield
         endif								! Marco 4Apr2005 - Output yield

		 
		 if (opt.eq.7) then					! Marco 4Apr2005 - Output yield
			
			write(1022,*) '   vnucl       1 ', tsnaps(count), '  1   1   0'	! Marco 4Apr2005 - Output yield
            ...
            write(21,*)'1 0 '				! Marco 4Apr2005 - Output yield
            write(21,*)'1 1 '				! Marco 4Apr2005 - Output yield
            write(21,*)'Vnucl, NO_UNITS'	! Marco 4Apr2005 - Output yield
            
            do i = 1,nnode					! Marco 4Apr2005 - Output yield
               write(21,'(I8,2X,E14.6)')&	! Marco 4Apr2005 - Output yield
                    i,vnucl_read(i)			! Marco 4Apr2005 - Output yield
			   write(1022,'(I8,2X,E14.6)')&	! Marco 4Apr2005 - Output yield
                    i,vnucl_read(i)			! Marco 4Apr2005 - Output yield
            enddo							! Marco 4Apr2005 - Output yield
	  ...
	  deallocate(yield_read)				! Marco 4Apr2005 - Output yield
	  deallocate(vnucl_read)				! Marco 4Apr2005 - Output yield
      deallocate(sxx,syy,szz,sxy)
      deallocate(spr,svm)
	  deallocate(duxdx_average,duydx_average,duxdy_average,duydy_average)
...
!
      ...
      lname = len_trim(file_name)
	  in_fileS4 = file_name(1:lname) // '4_xxx_xxx.bin'				!sigma_xx
	  in_fileS5 = file_name(1:lname) // '5_xxx_xxx.bin'             !sigma_yy
	  in_fileS6 = file_name(1:lname) // '6_xxx_xxx.bin'				!sigma_xy
      in_fileS7 = file_name(1:lname) // '7_xxx_xxx.bin'				!sigma_zz	!Clara 12/01/2005
...
!********************************************************************
      subroutine LEG(s,ak,ss,p1,D,DV,dt,csi,bef,pc,u,ff)
!********************************************************************
!     © POLIMI, 2004, All Rights Reserved
!     Authors: Clara Zambelli (28//10/2004)
      ...
      real*8, dimension(3,3) :: S1,ET,SET,SS
	  integer*4 :: i,j,k
      
	  real*8, dimension(14) ::u 
	  real*8 :: gam,gav,ga,z51,ff,dt
	  real*8 :: term1,term2	!Marco 18May2005 - Speed-up changes 6% improvement
...
!     calcolus of plastic potential
      ...
      term1 = 3.*ga*SQ3/P1			!Marco 18May2005 - Speed-up changes 6% improvement
	  term2 = 9./2.*(ga-1.)*SQ3/P1	!Marco 18May2005 - Speed-up changes 6% improvement

      do i = 1,3
	    do j = 1,3
	   	  SS(i,j) = Z51*AK(I,J)- term1*SET(I,J) + term2*ET(I,J)	!Marco 18May2005 - Speed-up changes 6% improvement
        enddo
      enddo
...
!      if(ff.le.(0.1))then
!	    write(*,*)ff
!	    vnucl=gam*p1*exp(gav*FF)
!      else
!	    write(*,*)ff
!         vnucl=21.99011*((log(13.88007*ff))**2.)*p1
!	  endif
...
!     Authors: Clara Zambelli (28//10/2004)
     ...
     do i = 1,3
        ...
        enddo
      enddo
!	 write(*,*)aw(1,1),aw(2,2),ak(1,1),ak(2,2)
...
! Scalar product of two arrays A(n,n) , B(n,n) ----> C
	  ...
	  C = A(1,1)*B(1,1) +  A(1,2)*B(2,1) + A(1,3)*B(3,1) + &	!Marco 18May2005 - Speed-up changes 6% improvement
	      A(2,1)*B(1,2) +  A(2,2)*B(2,2) + A(2,3)*B(3,2) + &	!Marco 18May2005 - Speed-up changes 6% improvement
		  A(3,1)*B(1,3) +  A(3,2)*B(2,3) + A(3,3)*B(3,3)		!Marco 18May2005 - Speed-up changes 6% improvement
...
! from vector A(2n) to array B(n,n)
	  ...
	  B(1,1) = A(1)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(1,2) = A(4)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(1,3) = A(5)	!Marco 18May2005 - Speed-up changes 6% improvement

	  B(2,1) = A(4)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(2,2) = A(2)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(2,3) = A(6)	!Marco 18May2005 - Speed-up changes 6% improvement

	  B(3,1) = A(5)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(3,2) = A(6)	!Marco 18May2005 - Speed-up changes 6% improvement
	  B(3,3) = A(3)	!Marco 18May2005 - Speed-up changes 6% improvement
...
!********************************************************************
   	subroutine PAR(dens,u,&
		 ga_l,la_l,bef0_l,csi_l,psi_l,gw_l,po_l,pq_l,pw_l,bm_l,befl_l,gam_l,gav_l,dvisco_l,&              !Clara-new definition nucleus viscous
		 ga_d_l,la_d_l,bef0_d_l,csi_d_l,psi_d_l,gw_d_l,po_d_l,pq_d_l,pw_d_l,bm_d_l,befl_d_l,gam_d_l,gav_d_l,dvisco_d_l)	!Marco 18May2005 - Speed-up changes 6% improvement
!********************************************************************
!     © POLIMI, 2004, All Rights Reserved
!     Authors: Clara Zambelli (28//10/2004)
!
! 
	  ...
	  real*8 :: dens
      real*8 :: densr1,densr1_80
      real*8 :: ga_l,la_l,bef0_l,csi_l,psi_l,gw_l,po_l,pq_l,pw_l,bm_l,befl_l,gam_l,gav_l,dvisco_l
	  real*8 ::	ga_d_l,la_d_l,bef0_d_l,csi_d_l,psi_d_l,gw_d_l,po_d_l,pq_d_l,pw_d_l,bm_d_l,befl_d_l,gam_d_l,gav_d_l,dvisco_d_l	!Marco 18May2005 - Speed-up changes 6% improvement

!	  real*8 :: ga,la,bef0,csi,psi,gw,po,pq,pw,bm,befl,gam,gav,dvisco
      ...
      !ga=ga_l+(ga_d_l)*(densr1_80) 
      !la=la_l+(la_d_l)*(densr1_80)   
      !bef0=bef0_l+(bef0_d_l)*(densr1_80) 
      !csi=csi_l+(csi_d_l)*(densr1_80) 
      !psi=psi_l+(psi_d_l)*(densr1_80) 
      !gw=gw_l+(gw_d_l)*(densr1_80) 
      !po=po_l+(po_d_l)*(densr1_80) 
      !pq=pq_l+(pq_d_l)*(densr1_80) 
      !pw=pw_l+(pw_d_l)*(densr1_80) 
      !bm=bm_l+(bm_d_l)*(densr1_80) 
      !befl=befl_l+(befl_d_l)*(densr1_80) 
      !gam=gam_l+(gam_d_l)*(densr1_80)                   
      !gav=gav_l+(gav_d_l)*(densr1_80)   

!    identificazione dei parametri
      u(1)=ga_l+(ga_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(2)=la_l+(la_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(3)=bef0_l+(bef0_d_l)*(densr1_80)	!Marco 18May2005 - Speed-up changes 6% improvement
      u(4)=csi_l+(csi_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(5)=gw_l+(gw_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement 
      u(6)=po_l+(po_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(8)=pq_l+(pq_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(9)=bm_l+(bm_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(10)=pw_l+(pw_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(11)=befl_l+(befl_d_l)*(densr1_80)	!Marco 18May2005 - Speed-up changes 6% improvement
      u(12)=psi_l+(psi_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(13)=gam_l+(gam_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(14)=gav_l+(gav_d_l)*(densr1_80)		!Marco 18May2005 - Speed-up changes 6% improvement
      u(15)=dvisco_l+(dvisco_d_l)*(densr1_80)   !Clara-new definition nucleus viscous
...
!     Mexico _A_
	  ...
	  real*8, dimension(nnode*2) :: yield_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(nnode*2) :: vnucl_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(nnode*2) :: vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read ! Clara 23/04/05
	  character*70 :: file_name
	  character*70 :: in_file_u1,in_file_u0
	  character*70 :: in_file_Fe,in_file_Fk
	  character*70 :: in_file_vivel
	  character*70 :: in_file_yield ! Marco 4Apr2005 - Output yield
	  character*70 :: in_file_vnucl ! Marco 4Apr2005 - Output yield
	  character*70 :: in_file_vistn1_0,in_file_vistn2_0,in_file_vistn3_0,in_file_vistn4_0  ! Clara 23/04/05
	  real*8 :: val
	  ...
	  in_file_yield = file_name(1:lname) // '_yield.bin' ! Marco 4Apr2005 - Output yield
	  in_file_vnucl = file_name(1:lname) // '_vnucl.bin' ! Marco 4Apr2005 - Output yield
      ...
      in_file_vistn4_0 = file_name(1:lname) // '_vistn4_0.bin'  ! Clara 23/04/05
	  ...
	  open(20,file=in_file_yield)				! Marco 4Apr2005 - Output yield        
      read(20,*)nd								! Marco 4Apr2005 - Output yield        
      do i = 1,nd								! Marco 4Apr2005 - Output yield
		read(20,*)in,val						! Marco 4Apr2005 - Output yield
		if (dabs(val).lt.1.d-30) val=0.d0   	! Marco 4Apr2005 - Output yield
		     yield_read(i) = val			    ! Marco 4Apr2005 - Output yield
	  enddo ! i									! Marco 4Apr2005 - Output yield        
      close(20)									! Marco 4Apr2005 - Output yield

	  open(20,file=in_file_vnucl)				! Marco 4Apr2005 - Output yield        
      read(20,*)nd								! Marco 4Apr2005 - Output yield        
      do i = 1,nd								! Marco 4Apr2005 - Output yield
		read(20,*)in,val						! Marco 4Apr2005 - Output yield
		if (dabs(val).lt.1.d-30) val=0.d0	    ! Marco 4Apr2005 - Output yield
	  ...
	  open(20,file=in_file_vistn4_0)            ! Clara 23/04/05           
      read(20,*)nd                              ! Clara 23/04/05         
      do i = 1,nd                               ! Clara 23/04/05
		read(20,*)in,val                        ! Clara 23/04/05
		if (dabs(val).lt.1.d-30) val=0.d0       ! Clara 23/04/05
		     vistn4_0_read(i) = val             ! Clara 23/04/05
	  enddo ! i                                 ! Clara 23/04/05  
      close(20)                                 ! Clara 23/04/05
...
!!     ********************************************************
      ...
      subroutine read_initial_condition_sand(nnode,file_name,nsnapsps,tsnapsps,&				! Clara SNAPPRES SAND
								 u1_read,u0_read,&												! Clara SNAPPRES SAND
								 Fe_read,Fk_read,&												! Clara SNAPPRES SAND
								 vivel_read,&													! Clara SNAPPRES SAND
								 yield_read,&													! Clara SNAPPRES SAND
...
! Clara SNAPPRES SAND
	  ...
	  real*8, dimension(nnode*2) :: vnucl_read ! Marco 4Apr2005 - Output yield
	  real*8, dimension(nnode*2) :: vistn1_0_read,vistn2_0_read,vistn3_0_read,vistn4_0_read ! Clara 23/04/05
	  real*8, dimension(nnode*2) :: a1n1_read,a1n2_read,a1n3_read,a1n4_read,a1n5_read,a1n6_read		    ! Clara SNAPPRES SAND
      real*8, dimension(nnode*2) :: epn1_read,epn2_read,epn3_read,epn4_read,epn5_read,epn6_read			! Clara SNAPPRES SAND
      real*8, dimension(nnode*2) :: z14n_read,u7n_read,densit_read										! Clara SNAPPRES SAND
	  character*70 :: file_name
	  character*70 :: in_file_u1,in_file_u0
	  character*70 :: in_file_Fe,in_file_Fk
	  character*70 :: in_file_vivel
	  ...
	  character*70 :: in_file_vnucl ! Marco 4Apr2005 - Output yield
	  character*70 :: in_file_vistn1_0,in_file_vistn2_0,in_file_vistn3_0,in_file_vistn4_0  ! Clara 23/04/05
	  character*70 :: in_file_a1n1,in_file_a1n2,in_file_a1n3,in_file_a1n4,in_file_a1n5,in_file_a1n6	 ! Clara SNAPPRES SAND
	  character*70 :: in_file_epn1,in_file_epn2,in_file_epn3,in_file_epn4,in_file_epn5,in_file_epn6	 ! Clara SNAPPRES SAND
	  character*70 :: in_file_z14n,in_file_u7n,in_file_densit									  ! Clara SNAPPRES SAND
	  real*8 :: val
	  ...
	  in_file_a1n1   = file_name(1:lname) // '_a1n1.bin'  ! Clara SNAPPRES SAND
	  in_file_a1n2   = file_name(1:lname) // '_a1n2.bin'  ! Clara SNAPPRES SAND
	  in_file_a1n3   = file_name(1:lname) // '_a1n3.bin'  ! Clara SNAPPRES SAND
	  in_file_a1n4   = file_name(1:lname) // '_a1n4.bin'  ! Clara SNAPPRES SAND
	  in_file_a1n5   = file_name(1:lname) // '_a1n5.bin'  ! Clara SNAPPRES SAND
	  in_file_a1n6   = file_name(1:lname) // '_a1n6.bin'  ! Clara SNAPPRES SAND
	  in_file_epn1   = file_name(1:lname) // '_epn1.bin'  ! Clara SNAPPRES SAND
	  in_file_epn2   = file_name(1:lname) // '_epn2.bin'  ! Clara SNAPPRES SAND
	  in_file_epn3   = file_name(1:lname) // '_epn3.bin'  ! Clara SNAPPRES SAND
	  in_file_epn4   = file_name(1:lname) // '_epn4.bin'  ! Clara SNAPPRES SAND
	  in_file_epn5   = file_name(1:lname) // '_epn5.bin'  ! Clara SNAPPRES SAND
	  in_file_epn6   = file_name(1:lname) // '_epn6.bin'  ! Clara SNAPPRES SAND
	  in_file_z14n   = file_name(1:lname) // '_z14n.bin'  ! Clara SNAPPRES SAND
	  in_file_u7n    = file_name(1:lname) // '_u7n.bin'   ! Clara SNAPPRES SAND
	  in_file_densit = file_name(1:lname) // '_densit.bin'! Clara SNAPPRES SAND
      ...
      open(20,file=in_file_a1n1)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n1_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

      open(20,file=in_file_a1n2)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n2_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_a1n3)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n3_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_a1n4)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n4_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_a1n5)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n5_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_a1n6)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     a1n6_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn1)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn1_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn2)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn2_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn3)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn3_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn4)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn4_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn5)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn5_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_epn6)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     epn6_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_z14n)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     z14n_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_u7n)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     u7n_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND

	  open(20,file=in_file_densit)                    ! Clara SNAPPRES SAND          
      read(20,*)nd									! Clara SNAPPRES SAND
      do i = 1,nd									! Clara SNAPPRES SAND
	    read(20,*)in,val							! Clara SNAPPRES SAND
		if (dabs(val).lt.1.d-30) val=0.d0			! Clara SNAPPRES SAND	
		     densit_read(i) = val						! Clara SNAPPRES SAND
	  enddo ! i										! Clara SNAPPRES SAND
      close(20)										! Clara SNAPPRES SAND
      
      return
      
      end subroutine read_initial_condition_sand
...
!     
      ...
      do i = 1,nu
		 !write(20,*)ui(i),uv(i)
         write(20,'(I12,1X,SP,E50.43E3)')ui(i),uv(i)
		 !write(20,'(I12,1X,SP,E10.3E3)')ui(i),uv(i)
      enddo
...
!     ********************************************************
      ...
      subroutine dime_monitorsl(nn_s,xx_s,yy_s,& ! Marco - MONITOR LINE - 14Feb2005
								num_monitorsl_sum,&  ! Marco - MONITOR LINE - 14Feb2005
								nmonitorsl,x1_monitorl,y1_monitorl,& ! Marco - MONITOR LINE - 14Feb2005
								x2_monitorl,y2_monitorl) ! Marco - MONITOR LINE - 14Feb2005
...
!     
      ...
      integer*4 :: nn_s,i,j
      real*8, dimension(nn_s) :: xx_s,yy_s
	  integer*4 :: nmonitorsl
	  real*8, dimension(nmonitorsl) :: x1_monitorl,y1_monitorl,x2_monitorl,y2_monitorl
	  !integer*4, dimension(nmonitorsl) :: num_monitorsl
	  integer*4 :: num_monitorsl_sum
	  real*8 :: cos_u,cos_v,tol,val

	  tol = 10.0e-010
	  num_monitorsl_sum = 0
      
      do i = 1,nmonitorsl
		 cos_u = x1_monitorl(i) - x2_monitorl(i)
		 cos_v = y1_monitorl(i) - y2_monitorl(i)
		 !num_monitorsl(i) = 0
		 
	     do j = 1,nn_s
			val = abs((xx_s(j)-x1_monitorl(i))*cos_v - (yy_s(j)-y1_monitorl(i))*cos_u) / &
				  (cos_u**2 + cos_v**2)**.5

		    if (val.le.tol) then

				num_monitorsl_sum = num_monitorsl_sum + 1
      ...
      end subroutine dime_monitorsl
      
      
      
!     ********************************************************
      
      subroutine read_monitorsl(nn_s,xx_s,yy_s,& ! Marco - MONITOR LINE - 14Feb2005
								!num_monitorsl,&  ! Marco - MONITOR LINE - 14Feb2005
								num_monitorsl_sum,&  ! Marco - MONITOR LINE - 14Feb2005
								x_monitorl,y_monitorl,& ! Marco - MONITOR LINE - 14Feb2005
								nmonitorsl,x1_monitorl,y1_monitorl,& ! Marco - MONITOR LINE - 14Feb2005
...
!     
      ...
      integer*4 :: nn_s,i,j,k
      real*8, dimension(nn_s) :: xx_s,yy_s
	  integer*4 :: nmonitorsl
	  integer*4 :: num_monitorsl_sum
	  real*8, dimension(nmonitorsl) :: x1_monitorl,y1_monitorl,x2_monitorl,y2_monitorl
	  !integer*4, dimension(nmonitorsl) :: num_monitorsl
	  real*8, dimension(num_monitorsl_sum) :: x_monitorl,y_monitorl 
	  real*8 :: cos_u,cos_v,tol,val
      ...
      end subroutine read_monitorsl
      
      
      
!     ****************************************************************
      
      subroutine dime_nonloc(nnode,xs,ys,node_index,cs_nnz,cs,&
                          nm,tag_mat,type_mat,sdeg_mat,tref_mat,prop_mat,&
                          ne,&
                          cs_nnz_bc,cs_bc,&
                          xx,yy,con_quad,nquad,con_line,nline,&
...
!     
      ...
      integer*4, dimension(0:cs_nnz_bc) :: cs_bc
	  integer*4 :: nn,im
	  integer*4 :: ie1,j1,i1,is1,in1,id1
	  integer*4 :: ie2,j2,i2,is2,in2,id2
	  ...
	  integer*4 :: nvp,ivp,i
	  real*8 :: valvp   ! Clara 
      integer*4, dimension(*) :: tagvp ! Clara

	  integer*4, dimension(nvp,nnode) :: end_nloc
	  integer*4, dimension(nvp,nnode) :: begin_nloc
	  integer*4, dimension(nvp,nnode) :: check_nloc_1
	  integer*4, dimension(nvp,nnode) :: check_nloc_2
	  ...
	  do ivp = 1,nvp
		do i = 1,nnode
			...
			end_nloc(ivp,i) = 0
			begin_nloc(ivp,i) = 0
			check_nloc_1(ivp,i) = 0
			check_nloc_2(ivp,i) = 0
      ...
      do im = 1,nm
		do ivp = 1,nvp
		 ...
		 if ((tagvp(ivp).eq.tag_mat(im)).and.(valvp.gt.0)) then 
         ...
         do ie1 = 1,ne
			if (cs(cs(ie1 -1) +0).eq.tag_mat(im)) then
				do j1 = 1,nn
					do i1 = 1,nn
						is1 = nn*(j1 -1) +i1
                        in1 = cs(cs(ie1 -1) +is1)
                              
                        if (node_index(in1).ne.0) then
							id1 = node_index(in1)

							if (check_nloc_1(ivp,id1).eq.0) then
							

							do ie2 = 1,ne
								if (cs(cs(ie2 -1) +0).eq.tag_mat(im)) then
									do j2 = 1,nn
										do i2 = 1,nn
											is2 = nn*(j2 -1) +i2
											in2 = cs(cs(ie2 -1) +is2)
                              
											if (node_index(in2).ne.0) then
												id2 = node_index(in2)

												if (check_nloc_2(ivp,id2).eq.0) then
													if (((xs(id1)-xs(id2))**2 + &
														(ys(id1)-ys(id2))**2).le.(valvp**2)) then

													
														end_nloc(ivp,id1) = end_nloc(ivp,id1) + 1
							...
							endif !(check_nloc_1(ivp,id1).eq.0) then

						endif !(node_index(in1).ne.0)
						check_nloc_1(ivp,id1) = 1
	...
	endif !((tagvp(ivp).eq.tag_mat(im)).and.(valvp(ivp,7).gt.0)) then 
	...
	do ivp = 1,nvp
	  do i = 1,nnode
		...
		if (i.eq.1) then
			begin_nloc(ivp,i) =  1
			end_nloc(ivp,i) =  end_nloc(ivp,i)
		else
			begin_nloc(ivp,i) = end_nloc(ivp,i-1) + 1
			end_nloc(ivp,i) = end_nloc(ivp,i) + end_nloc(ivp,i-1)
		endif
...
!     ****************************************************************
      ...
      subroutine create_connect_weigth_nonloc(nnode,xs,ys,node_index,cs_nnz,cs,&
                          nm,tag_mat,type_mat,sdeg_mat,tref_mat,prop_mat,&
                          ne,a1,b1,g1,a2,b2,g2,&
                          cs_nnz_bc,cs_bc,&
                          xx,yy,con_quad,nquad,con_line,nline,&
...
!     
	  ...
	  integer*4 :: nvp,ivp,i
	  real*8, dimension(nvp) :: valvp   ! Clara 
      integer*4, dimension(*) :: tagvp ! Clara
	  ...
	  integer*4 :: sum_end_nloc,i_nloc
	  integer*4, dimension(nvp,sum_end_nloc) :: connectivity_nloc
	  integer*4, dimension(nvp,sum_end_nloc) :: iq_nloc
	  integer*4, dimension(nvp,sum_end_nloc) :: ip_nloc
	  real*8, dimension(nvp,sum_end_nloc) :: weigth_nloc
	  real*8, dimension(nvp,nnode) :: integral_nloc
	  real*8 :: PI
	  real*8, dimension(nvp,nnode) :: sum_weigth

	  real*8, dimension(:), allocatable :: ct,ww	! dead line
      real*8, dimension(:,:), allocatable :: dd		! dead line
	  real*8 :: dxdx,dydx,dxdy,dydy	! dead line
	  real*8 :: radius,det_j
	  ...
	  do ivp = 1,nvp
		do i = 1,sum_end_nloc
			...
			connectivity_nloc(ivp,i) = 0
			weigth_nloc(ivp,i) = 0.0d0
      ...
      do im = 1,nm
		do ivp = 1,nvp
		 ...
		 if ((tagvp(ivp).eq.tag_mat(im)).and.(valvp(ivp).gt.0)) then 
		...
		allocate(ct(nn),ww(nn),dd(nn,nn))				! dead line
        call lgl(nn,ct,ww,dd)							! dead line
		!allocate(dxdx(nn),dydx(nn),dxdy(nn),dydy(nn))	! dead line
										...
										do i2 = 1,nn
											...
											if (node_index(in2).ne.0) then
												id2 = node_index(in2)
													...
													if (((xs(id1)-xs(id2))**2 + &
														(ys(id1)-ys(id2))**2).le.(valvp(ivp)**2)) then
														...
														if (check_nloc_2(ivp,id2).eq.0) then
															connectivity_nloc(ivp,end_nloc(ivp,id1)+begin_nloc(ivp,id1)) = id2
																														
															

																weigth_nloc(ivp,end_nloc(ivp,id1)+begin_nloc(ivp,id1)) = 1/((radius/5)*(2*PI)**.5) * &
																	exp(-((xs(id1)-xs(id2))**2 + (ys(id1)-ys(id2))**2)/(2*(radius/5)**2))
														...
														dxdy = b1(ie2) + g1(ie2) * ct(j2)
														dydy = b2(ie2) + g2(ie2) * ct(i2)
														dxdx = a1(ie2) + g1(ie2) * ct(i2)
														dydx = a2(ie2) + g2(ie2) * ct(j2)
														det_j = dxdx*dydy - dxdy*dydx
														det_j = dabs(det_j)

														

															!integral_nloc(ivp,id1) = integral_nloc(ivp,id1) + 1 * &
															!				 1/((radius/5)*(2*PI)**.5) * &
															!				 exp(-((xs(id1)-xs(id2))**2 + (ys(id1)-ys(id2))**2)/(2*(radius/5)**2))

															integral_nloc(ivp,id1) = integral_nloc(ivp,id1) + ww(i2) * ww(j2) * det_j * &
																			 1/((radius/5)*(2*PI)**.5) * &
																			 exp(-((xs(id1)-xs(id2))**2 + (ys(id1)-ys(id2))**2)/(2*(radius/5)**2))
														
														
														!integral_nloc(id1) = integral_nloc(id1) + ww(i2) * ww(j2) * det_j * &
														!					 1/((2*PI)**.5) * &
														!					 exp(-((xs(id1)-xs(id2))**2 + (ys(id1)-ys(id2))**2)/2)


														!integral_nloc(ivp,end_nloc(ivp,id1)+begin_nloc(ivp,id1)) = integral_nloc(ivp,end_nloc(ivp,id1)+begin_nloc(ivp,id1)) + &
														!																ww(i2) * ww(j2) * det_j
	...
	do ivp = 1,nvp
	    do i = 1,nnode
			do i_nloc = begin_nloc(ivp,i),end_nloc(ivp,i)	
				weigth_nloc(ivp,i_nloc) = weigth_nloc(ivp,i_nloc) / &
											integral_nloc(ivp,connectivity_nloc(ivp,i_nloc))
			enddo
		enddo
	enddo
	 ...
	 !deallocate(dxdx,dydx,dxdy,dydy)	! dead line
      
     return
     end subroutine create_connect_weigth_nonloc
                  
      
      
! *********************************************************
      
      subroutine find_point_pos(ne,alfa1,beta1,gamma1,&
								alfa2,beta2,gamma2,&
								delta1,delta2,&
								xt,yt,el,xr,yr)
      
!     © CRS4, 2003, All Rights Reserved
!     Authors: Luca Massidda
!     
!     Last revision: 04-03-2004
      ...
      do ie = 1,ne
		...
		dxdx = alfa1(ie) + gamma1(ie) * jr
		dydx = alfa2(ie) + gamma2(ie) * jr
	
		dxdy = beta1(ie) + gamma1(ie) * ir
		dydy = beta2(ie) + gamma2(ie) * ir
        ...
        det_j = dxdx * dydy - dxdy * dydx
            ...
            !fx = alfa11(ie)*ir + alfa12(ie)*jr &
            !   + alfa13(ie)*kr + beta11(ie)*jr*kr &
            !   + beta12(ie)*ir*kr + beta13(ie)*ir*jr &
            !   + gamma1(ie)*ir*jr*kr + delta1(ie) -xt
            
            !fy = alfa21(ie)*ir + alfa22(ie)*jr &
            !   + alfa23(ie)*kr + beta21(ie)*jr*kr &
            !   + beta22(ie)*ir*kr + beta23(ie)*ir*jr &
            !   + gamma2(ie)*ir*jr*kr + delta2(ie) -yt

			fx = alfa1(ie)*ir + beta1(ie)*jr &
               + gamma1(ie)*ir*jr + delta1(ie) -xt
            
            fy = alfa2(ie)*ir + beta2(ie)*jr &
               + gamma2(ie)*ir*jr + delta2(ie) -yt

            
            dir = -(a11*fx +a21*fy) / det_j
            djr = -(a12*fx +a22*fy) / det_j
