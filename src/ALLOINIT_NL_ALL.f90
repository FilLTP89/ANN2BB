...
subroutine ALLOINIT_NL_ALL(ne,sdeg_mat,nm,nnt,cs_nnz,cs,u1,u2,vel,acc,v1,fk,fe,fd,&
    duxdx,duxdy,duydx,duydy,sxx,syy,szz,sxy,xkin,riso,epl,option_out_var,nl_sism,sism,&
    ...
    real*8, dimension(:,:,:), allocatable,intent(inout)     :: sxx,syy,szz,sxy,riso
    real*8, dimension(:,:,:), allocatable,intent(inout)     :: duxdx,duxdy,duydx,duydy
    real*8, dimension(:,:,:,:), allocatable,intent(inout)   :: xkin,epl

    ! counters
    integer*4                                               :: nn,im,iaz,ie,in,is,i,j
    integer*4, dimension(ne)                                :: idx
    ! GLL number at each element
    do ie = 1,ne
        im = cs(cs(ie-1)+0)
        nn = sdeg_mat(im)+1
        idx (ie) = nn
    ...
    allocate(xkin(4*nnt));   xkin_all = 0.0d0
    allocate(epl(4*nnt));     epl_all = 0.0d0
    allocate(riso(nnt));     riso_all = 0.0d0
