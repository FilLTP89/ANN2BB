...
subroutine UPDATE_NL_ALL(ie,nnt,nn,cs_nnz,cs,fk,mvec,sxx,syy,szz,sxy,&
    ...
    do j = 1,nn
        do i = 1,nn
            ...
            sxx(in)                 = sxx(in) + sxx_el(i,j)
            syy(in)                 = syy(in) + syy_el(i,j)
            szz(in)                 = szz(in) + szz_el(i,j)
            sxy(in)                 = sxy(in) + sxy_el(i,j)
            duxdx(in)               = duxdx(in) + duxdx_el(i,j)
            duydy(in)               = duydy(in) + duydy_el(i,j)
            duxdy(in)               = duxdy(in) + duxdy_el(i,j)
            duydx(in)               = duydx(in) + duydx_el(i,j)
            xkin_all(in)            = xkin_all(in)      + xkin_el(1,i,j)
            xkin_all(in+nnt)        = xkin_all(in+nnt)  + xkin_el(2,i,j)
            xkin_all(in+2*nnt)      = xkin_all(in+2*nnt)+ xkin_el(3,i,j)
            xkin_all(in+3*nnt)      = xkin_all(in+3*nnt)+ xkin_el(4,i,j)
            riso_all(in)            = riso_all(in) + riso_el(i,j)
