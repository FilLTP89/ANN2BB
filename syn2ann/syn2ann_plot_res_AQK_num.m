...
xlm.tha = {[0;20];[0;20];[0;20]};
ylm.tha = {[-.6,.6];[-.6,.6];[-.6,.6]};
% xtk.tha = {0:10:60;0:10:60;0:10:60};
xtk.tha = {[0:5:20];[0:5:20];[0:5:20]};
xlb.tha = {'t [s]'};
...
%         fn = fullfile(sp,sprintf('%s_rec_org_flt_num',st));
%         syn2ann_plot_compare(ones(5,1),{rec.org},nm(i_),{'ORIGINAL'},(cpp{j_}),fn,0);
%         
        %% *POST-PROCESS - NUMERICAL SIMULATIONS VS RECORDS*
        fn = fullfile(sp,sprintf('%s_sim_rec_org_num',st));
        syn2ann_plot_compare(ones(5,1),{rec.org;nss.hyb},nm(i_),...
            {'RECORDED';'SIMULATED'},(cpp{j_}),fn,[0,2]);
%         
%         %% *POST-PROCESS - BB HYBRIDIZATION*
%         fn = fullfile(sp,sprintf('%s_sim_sp96_hyb_num',st));
%         syn2ann_plot_compare([0 0 1 1 1],{nss.hyb;sps.hyb;hbs},nm(i_),...
%             {'SIMULATED';'SP96';'HYBRID'},(cpp{j_}),fn,2*ones(3,1));
...
%             {'none','none','none','none','none'},{'--','--','-','-','-'});
        ...
        fn = fullfile(sp,sprintf('%s_hyb_ann_num',st));
        ...
        fn = fullfile(sp,sprintf('%s_spm_ann_num',st));
        ...
        fn = fullfile(sp,sprintf('%s_spm_rec_num',st));
        syn2ann_plot_compare([1 1 1 1 1],{rec.org,spm.(cpp{j_})},nm(i_),...
            {'RECORDED';'MATCHED'},(cpp{j_}),fn,[0,2]);
        
        %         %% *POST-PROCESS - HYBRIDS VS RECORDS*
        %         fn = fullfile(sp,sprintf('%s_hyb_rec_num',st));
