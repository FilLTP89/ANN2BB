...
function [varargout] = syn2ann_write_maps(varargin)
    ...
    if any(strcmpi(mon.map.typ,'pgv'))
        for i_=1:mon.na
            for j_=1:mon.nc
                map_spm.(strcat('psa_',mon.cp{j_})){i_,1} = ...
    ...
    if any(strcmpi(mon.map.typ,'psa'))
        for k_=1:numel(mon.map.vTn.psa)
            ...
            for i_=1:mon.na
                for j_=1:mon.nc
                    map_spm(i_).(strcat('psa_',mon.cp{j_},'_',num2str(vTn))) = ...
                        abs(spm.sps.(mon.cp{j_}).syn{i_}.psa.(mon.cp{j_})(idx));
                end
                map_spm(i_).(strcat('psa_gh_',num2str(vTn))) =...
            ...
            super_csvwrite(sprintf('%s_spm_psa_%u_%u.csv',...
                ...
                [map_spm.X,map_spm.Y,map_spm.psa_ew,map_spm.psa_ns,...
                map_spm.psa_gh],...
                '%20.3f,%20.3f,%20.3f,%20.3f,%20.3f,%20.3f,%20.3f\n',...
                {'E-UTM [m]','N-UTM [m]','Rew [m/s2]','Rns [m/s2]','Rgh [m/s2]'});
            super_csvwrite(sprintf('%s_hyb_psa_%u_%u.csv',...
                mon.map.fnm,round(mon.map.vTn.psa(k_)*100),NJB),...
                [map_hyb.X,map_hyb.Y,map_hyb.psa_ew,map_hyb.psa_ns,...
                map_hyb.psa_gh],...
                '%20.3f,%20.3f,%20.3f,%20.3f,%20.3f,%20.3f,%20.3f\n',...
                {'E-UTM [m]','N-UTM [m]','Rew [m/s2]','Rns [m/s2]','Rgh [m/s2]'});
        end
