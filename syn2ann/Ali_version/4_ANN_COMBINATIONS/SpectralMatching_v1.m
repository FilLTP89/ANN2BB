...
%%%%%%%%%%%%%%%%%%%%
function  [out_t,out_acc,out_vel,out_dis,out_T,out_Se,out_freq,out_FAS,n_] = CORREZIONE(target_Se,acc_,niter,tol_u,tol_l)   
...
Sp_in2=Sp_in; %added by AGO on 15/12
...
% added by AGO on 15/12
if T_in(i)<T_sc
if Sp_acc(i)>3*Sp_in(i)
    mult(i)=(1+0.70*tol_upp);
    Sp_in(i)=Sp_in(i)*mult(i);
else
    if Sp_acc(i)<0.333*Sp_in(i)
        mult(i)=(1-0.70*tol_low);
        Sp_in(i)=Sp_in(i)*mult(i);
    else
       logr=log10(Sp_acc(i)/Sp_in(i));
       mult(i)=((1-0.70*tol_low)+((1+0.70*tol_upp)-(1-0.70*tol_low))*...
...
%             % Commented by AGO on 15/12
% %             if ((rat>1 && rat <= 1+tol_upp && Sp_acc(i)/Sp_in2(i)>1 )|| ...
% %                 (rat<1 && rat >= 1-tol_low && Sp_acc(i)/Sp_in2(i)<1)) ...
% %                  && T_in(i) < T_sc
% %                  rat=1;
% %             end
%             if T_in(i) < T_sc
%                  if(Sp_acc(i)/Sp_in2(i)>1)
%                      if(rat>1 && rat <= 1+tol_upp)
%                          rat=1;
%                      else
%                          rat=1+tol_upp;
%             end
...
acc_pro = tukeywin(length(acc_pro),0.20).*acc_pro;  % added by Maria on 5/12/2016
% end of modification by AGO on 15/12

for times=1:2  % added by AGO on 18/11/2016 repeating the PGA
...
if pga < (1-tol_low_pga)*abs(pga_target) || ...
        pga > (1+tol_upp_pga)*abs(pga_target) 
...
% modified by AGO on 15/12
acc_pro(idx2+1)=acc_pro(idx2+1)+acc_corr/2;
acc_pro(idx2+2)=acc_pro(idx2+2)+acc_corr/2;
% end of modification by AGO on 15/12
...
for i=2:length(T_in)
        ...
        rat_real(i)=Sp_acc_pro_final(i)/Sp_in(i);
        if T_in(i)<=0.75
            if rat_real(i)<1+tol_upp && rat_real(i)>1-tol_low
            n_=n_+1;
            end
        else
            if rat_real(i)<1.10 && rat_real(i)>0.90
            n_=n_+1;
            end
        end
end
