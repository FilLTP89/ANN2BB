...
!> @param[out] IDG_TOTAL, JDG_TOTAL, MDG_TOTAL  dg matrix in sparse morse format
     ...
     subroutine MAKE_DG_SPARSE_TOTAL(nelem_dg, el_new, nnz_dg_total, cs_nnz, cs, &
     ...
     do i = 1, nelem_dg
        ...
        allocate(IDG_PLUS(el_new(i)%nnz_plus),IDG_MINUS(el_new(i)%nnz_minus))
        
        do j = 1, 2*(el_new(i)%deg)**2 
          
           do k = el_new(i)%IPLUS(j-1) + 1, el_new(i)%IPLUS(j)
               IDG_PLUS(k) = j
           enddo
           do k = el_new(i)%IMIN(j-1) + 1, el_new(i)%IMIN(j)
               IDG_MINUS(k) = j
           enddo
        enddo
        
        do j = 1,  el_new(i)%nnz_plus
           IDG_TOTAL(i_total) = nodes_plus(IDG_PLUS(j))
           JDG_TOTAL(i_total) = nodes_plus(el_new(i)%JPLUS(j))
           MDG_TOTAL(i_total) = el_new(i)%matPlus(j)
           i_total = i_total + 1
        enddo   
        
        do j = 1,  el_new(i)%nnz_minus
           IDG_TOTAL(i_total) = nodes_plus(IDG_MINUS(j))
           JDG_TOTAL(i_total) = nodes_minus(el_new(i)%JMIN(j))
           MDG_TOTAL(i_total) = el_new(i)%matMin(j)
           i_total = i_total + 1
        enddo   
    ...
    enddo       
     ...
     end subroutine MAKE_DG_SPARSE_TOTAL
